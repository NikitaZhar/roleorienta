...
Дата: 2026-09-11 (ревизии: 2026-09-22; 2026-09-25 — ADR-19, ADR-20)
Статус: рабочий документ. Ревизия 2026-09-22: охват расширен на вендор-адаптеры, включая энтерпрайз-ATS (Workday, SAP SuccessFactors, SmartRecruiters); отличие продукта уточнено (курирование, честность покрытия и мониторинг, а не эксклюзивный доступ к лентам) — см. ADR-17, ADR-18; исправлено рассогласование §4/§6 с ADR-10 (брокер и outbox — с Этапа 1). Приоритет Этапа 1 пересмотрен в §14. **Ревизия 2026-09-25 (ADR-19, ADR-20; действует, отменяет расходящиеся с ней части прежних ревизий):** обнаружение — от компаний региона к карьерной странице, полностью автоматически, без очереди подтверждения человеку; признак покрытия («скрытая») исключён; ручной ввод компании — третий источник по желанию клиента; порядок работ — план развития (`docs/roleorienta-plan-korrektirovka.md`), §14.
---
...
| Термин | Значение |
|---|---|
| **Источник** (`Source`) | Интеграция с конкретной системой найма или карьерной страницей компании (площадки — только источник названий компаний, ADR-20): адрес, тип, расписание, лимиты |
...
| **Кандидат в работодатели** (`EmployerCandidate`) | Найденная компания/источник, ещё не прошедшие автоматические проверки |
| **Признак покрытия** | *Исключён (ADR-20).* Ранее — метка «скрытая»; построенный код удаляется отдельным срезом |
| **Технологический профиль компании** (`CompanyTechProfile`) | Производный агрегат технологий по всем вакансиям работодателя |
| **Гейт уверенности** | Порог, ниже которого обнаруженный источник не подключается; неуверенное пропускается или перепроверяется позже, без очереди человеку (ADR-20) |
...
|---|---|
| `User` | Учётная запись |
| `Company` | Компания-работодатель. Карьерные источники связываются через `CompanySource`, а не напрямую (одна доска может обслуживать несколько юрлиц) |
...
| `Source` | Конкретный источник сбора: доска работодателя (система найма или собственная страница с `JobPosting`). Адрес, расписание, лимиты, версия адаптера, состояние. Ссылается на `Provider`; принадлежность одной компании необязательна |
| `CompanySource` | Проверенная связь «источник → работодатель»; многие-ко-многим — для доски, обслуживающей несколько юрлиц (например, тенант с брендами) |
...
| `Vacancy` | Объединённая вакансия |
| `VacancyRevision` | Изменения значимых полей (история наблюдений приложения, с первого сбора) |
| `CoverageAssessment` | *Исключено (ADR-20).* Таблица V29 остаётся в схеме до среза удаления кода |
...
| `MarketProfile` | Конфигурация рынка/ниши: правовой режим зарплаты (обязательна ли публикация по закону), набор мейнстрим-площадок региона (для покрытия — к удалению, ADR-20; для названий компаний — план развития E3, по разрешению). Это настройка, а не пользовательские данные |
...
- **Вакансия объединяет публикации при надёжном общем признаке** (тот же работодатель + та же система найма/ID, либо совпадение по безопасно нормализованному URL); а с Этапа 2 — дополнительно **обязательным минимальным нечётким сопоставлением** (работодатель + нормализованный заголовок + близость дат публикации, с порогом уверенности; сомнительные пары не сливаются и остаются отдельными записями, ручной проверки нет — ADR-20). Иначе публикации остаются отдельными записями. Ссылки и происхождение сохраняются.
- **Признака покрытия («скрытая») нет (ADR-20).** Вакансия существует, только если она есть на карьерной странице компании; сравнение с площадками не ведётся. Построенные `CoverageAssessment` (V29), параметр и поле `coverage` в API ленты и строка покрытия в дайджесте удаляются отдельным срезом.
...
**Контракт адаптера.** Каждый `Provider` (система найма или общий адаптер `JobPosting`) реализует единый интерфейс: получить список публикаций **в области источника** — доски конкретной компании, — с пагинацией; получить одну публикацию; вернуть структурированные поля в общем виде. Один адаптер обслуживает все компании на этой системе. Версия адаптера привязана к типу `Provider` и участвует в версионировании парсера (раздел 8).

**Типы систем найма (примеры целей адаптеров; точный набор — по частоте типов страниц, план развития K3–K4).**
...
Площадки-агрегаторы — не источник вакансий, а источник названий компаний (план развития, E3; только при разрешении их условий, ADR-20). Единый и предсказуемый формат адреса лент — это ещё и то, что делает автоматическое обнаружение посильным (адрес выводится по шаблону из домена и типа системы). **Охват — по вендорам, а не по одной универсальной странице (ADR-17):** каждый ATS-вендор (SMB и энтерпрайз) — это один адаптер за общим контрактом; за пределами охвата остаётся только произвольная кастомная карьерная страница без вендорного шаблона и без разметки schema.org `JobPosting` (ADR-19; её сбор — дорогой и без гарантий, non-goal). Энтерпрайз-ленты (Workday, SuccessFactors) читаемы, но их доступность и правовое основание проверяются **по-тенантно** (A24), а часть SuccessFactors-витрин требует рендеринга страницы, а не готовой ленты.

**Contract-карточка адаптера (A12).** Приведённые адреса — ориентиры; точные контракты проверяются при подключении вендора (план развития, K4) по официальной документации, потому что они различаются и меняются. Каждый `Provider` описывается capability-карточкой: list/detail, виды пагинации, языки представления, обязательность credentials, полное/частичное перечисление, включение скрытых записей, семантика дат, условные запросы. Известные нюансы, которые нельзя игнорировать:
...
**Слой обнаружения работодателей** — отдельный, независимый от сбора. Задача: `найти компанию → найти её сайт → найти карьерную страницу и определить её тип → поставить на регулярный сбор`. **Работает полностью автоматически (ADR-20):** уверенно распознанная и подтверждённая карьерная страница подключается как `Source` сама; неуверенное (сайт, страница, принадлежность, лента) человеку не передаётся — компания пропускается или перепроверяется позже. Нет карьерной страницы, пригодной для структурного чтения (система найма с лентой или разметка schema.org `JobPosting`), — компания пропускается, ссылка не хранится; простая страница «Карьера» без разметки не анализируется. Карьерные страницы периодически перепроверяются (раз в месяц и сразу при пропаже или смене типа); robots.txt сайта соблюдается.
...
**Источники компаний (входы обнаружения) — по порядку (ревизия 2026-09-25, ADR-19, ADR-20):** (1) открытые реестры юрлиц и индексы систем найма — ядро; (2) площадки-агрегаторы как источник названий компаний — только при разрешении их условий; (3) ручной ввод компании — только третьим вариантом, опция по желанию клиента. Входы:

- **От региона к карьерной странице — основной вход (ADR-19, ADR-20).** (1) Компании региона порциями — из открытых реестров юрлиц **без фильтра по виду деятельности и размеру** (Словакия — RPO; Австрия — Firmenbuch HVD); очередь с курсором, найти сразу все компании региона не требуется; (2) сайт компании — метод поиска выбирается исследованием на выборке (кандидат домена по названию, открытые каталоги вроде Wikidata, поисковый API — план развития, K2); принадлежность сайта подтверждается обязательными реквизитами на нём (IČO в Словакии, номер Firmenbuch в выходных данных в Австрии); (3) карьерная страница на сайте и её тип: вендорная система найма (по ссылкам и адресам — адаптер вендора) или собственная страница с разметкой schema.org `JobPosting` (общий адаптер); иначе компания пропускается; (4) отбор вакансий по нише — по заголовку и описанию вакансии, а не по профилю компании.

- **Гарвест ATS-хостов из публичных датасетов — дополнительный вход** (до 2026-09-25 — основной; построен для Workday и Personio, project-notes §55, §91). Common Crawl и Certificate Transparency дают **разные типы кандидатов и не взаимозаменяемы (A11):**
...
- **Резолв по домену компании — часть основного входа (шаг 3).** Имея подтверждённый домен компании, пробуем известные шаблоны ATS для домена и его редиректов и валидируем ленту.
- **Расширение по связям — не входит в ядро** (более шумный вход; рассматривается после ядра). Переход от известных компаний к связанным (портфели фондов, отраслевые каталоги, «похожие компании»).
- **Площадки-агрегаторы — второй источник названий компаний** (план развития, E3): только при разрешении или договоре; условия karriere.at и profesia.sk автоматическое чтение запрещают (project-notes §90). Вакансии площадок не собираются — вакансии берутся только с карьерной страницы компании.
- **Ручной ввод компании — третий источник, опция по желанию клиента** (план развития, E4): клиент называет компанию (название или сайт); дальше — тот же автоматический путь и те же проверки, без ручного подтверждения.

Любой вход даёт лишь *кандидата*: каждый проходит резолв системы найма, валидацию ленты, **проверку принадлежности доски работодателю** (A11 — не только валидность JSON/XML; для Workday — тенант принадлежит клиенту, подтверждение по описанию страницы доски и отсутствию признаков кадрового агентства, project-notes §79), гейт уверенности (неуверенное — пропуск или перепроверка, не очередь человеку) и SSRF-защиту (§9), а также **дедупликацию работодателей** (одна компания не заводится дважды из разных входов). Порядок работ по входам и вендорам — план развития (`docs/roleorienta-plan-korrektirovka.md`, K1–K4, E1–E4); первая страна — Словакия.
...
| Задание | Что делает |
|---|---|
| `DISCOVER_EMPLOYER` | Найти компанию-кандидата, определить источник/систему найма; при уверенном резолве создать/обновить `Source`, иначе — пропуск или перепроверка позже (ADR-20; построенная очередь подтверждения — к замене) |
...
- **Дедупликация.** Точные признаки (§4): пара «источник + внешний ID», нормализованный URL, подтверждённый идентификатор работодателя/системы. Межисточниковое связывание одной вакансии (разные источники одной компании, например лента системы найма и собственная страница с `JobPosting`) — при надёжном общем признаке, а **с Этапа 2 — обязательным минимальным нечётким сопоставлением**: работодатель + нормализованный заголовок (регистр, пунктуация, стоп-слова) + близость дат публикации, с порогом уверенности; при превышении порога публикации связываются с сохранением обоих источников, сомнительные пары остаются отдельными записями — ни автослияния, ни ручной проверки (ADR-20). Полное нечёткое сопоставление по всему тексту — *(расширение)*.
...
|---|---|
| `GET /api/v1/vacancies` | Поиск и фильтры (зарплата, язык и др.; параметр `coverage` — построенный код, к удалению, ADR-20), курсорная пагинация |
| `GET /api/v1/vacancies/{id}` | Карточка с требованиями, источником и свежестью |
...
| `POST /api/v1/companies/suggest` | Ввести компанию — третий источник, опция по желанию клиента (план развития, E4; не реализовано; путь — рабочий) |
...
| `GET /api/v1/sources` | Источники и свежесть |
| `GET /api/v1/admin/employer-candidates` · `POST .../{id}/confirm` · `POST .../{id}/reject` | Очередь, подтверждение и отклонение кандидатов обнаружения — построенный код, к замене автоматическим подключением (ADR-20) |
| `POST /api/v1/admin/employer-candidates/discover` | Ручной запуск обнаружения по явной доске вендора — построенный код, к замене (ADR-20) |
...
- **Фильтры** (зарплата, язык, регион, формат, компания, даты, состояние) — по индексированным нормализованным столбцам; составные/частичные индексы под реальные комбинации, проверяются планами выполнения.
...
- **Недоверенный ввод.** Отчёты об ошибках валидируются, ограничиваются по частоте и **не меняют данные без решения администратора**. Компания, введённая клиентом (план развития, E4), валидируется, ограничивается по частоте и дальше проходит тот же автоматический путь и те же проверки (SSRF, принадлежность сайта), без ручного подтверждения (ADR-20). Внешний HTML очищается перед отображением.
- **Ограничение частоты аутентификации (по итогам разбора 2026-09-22).** `login`/`register` и пользовательские жалобы и ввод компаний ограничиваются по частоте (защита от брутфорса/credential-stuffing и спама) — обязательно до публичной экспозиции. Ротация идентификатора сессии при входе (`changeSessionId`) — против session fixation.
...
**Метрики:** время ответа API (p95, доля ошибок); возраст данных источника и время последнего успешного обхода; полные/частичные/неудачные обходы; скорость обработки, длина очереди, возраст задания; повторы, DLQ, задержка публикации outbox; новые/изменённые публикации, ошибки извлечения; обнаружение (найдено кандидатов, подключено автоматически, пропущено, отложено на перепроверку, доля ошибочных резолвов); агрегация техпрофилей (число пересчётов, задержка, доля компаний с недостаточной выборкой); реплики, CPU, память, ожидание бюджета источника.
...
Оба приложения (`job-api` и `job-worker`) присутствуют **с пилота**; worker с самого начала работает с брокером и transactional outbox (ADR-10), позже меняется только масштабирование (A25). Из таблицы состава этапа (§14) однозначно выводится список контейнеров, зависимостей и доступных функций.
...
- Подписка на компанию выдаёт её новые вакансии с карьерной страницы.
- Вакансия, опубликованная на карьерной странице компании (система найма или разметка `JobPosting`), доходит до выдачи; компания без такой страницы пропускается.
- Автоматически обнаруженный работодатель с уверенным резолвом появляется в источниках и даёт вакансии; неуверенный пропускается или перепроверяется позже — без очереди человеку.
...
- Дайджест содержит новые вакансии отслеживаемых компаний и дельты сохранённых; повторный сбор не создаёт повторных уведомлений.
...
| Ошибочный резолв обнаружения | Подать неоднозначного кандидата | Гейт уверенности не подключает мусорный источник; кандидат пропущен или отложен на перепроверку |
...
- Отдельная приёмка: ложные слияния, удаление снимка, корректировки администратора, репост, конфликт зарплаты между источниками.
...
**Порядок работ с 2026-09-25 (ADR-20) — план развития `docs/roleorienta-plan-korrektirovka.md`: сначала ядро** (компании региона → сайт → карьерная страница → вакансии → ниша и регион вакансии; надёжность без людей; единый отчёт; минимальный выход — список вакансий ниши и ежедневное письмо), **потом всё остальное.** Интерфейс, сложные фильтры, подписки, отклики и прочий «глянец» развиваются только после выполнения критерия «ядро чётко работает» (план, §3); построенное заранее заморожено. Этапы 0–5 ниже — история и инфраструктурная разбивка: их части о признаке покрытия («скрытая»), очереди подтверждения человеку, ручном вводе как дополнении и непустом первом экране **отменены** ADR-20; инфраструктурные этапы 3–4 идут после ядра. Колонка «Этап» в стеке (§3) читается с этой поправкой; «пилот» в документе — исторический термин построенного Compose-контура, а не отдельная цель.
...
- **Состав (пересмотрено 2026-09-22; порядок — по продуктовому риску).**
  ...
  *Приоритет A — ядро гипотезы (доделать первым):* **автоматический вход обнаружения** (гарвест ATS-хостов по вендорному шаблону из §5 — заменяет ручной seed; с 2026-09-25 — дополнительный вход, основной — от реестра, план развития K1; первый вендор — по Этапу 0, ориентир Workday через индекс Common Crawl); **проверка принадлежности доски работодателю** в гейте уверенности (A11 — не только валидность ленты); **дедупликация работодателей между входами**; адаптер(ы) вендоров выбранного входа (Workday / SuccessFactors / SmartRecruiters, §5); *(покрытие и фильтр «только скрытые» — построено в §81–§86, исключено ADR-20)*; `SourceSnapshot` (S3) с привязкой `Requirement`→снимок (объяснимость, A18/ADR-8); дайджест изменений + email.
  ...
  *Обязательно уже в пилоте (по итогам аудита A01–A29):* закрытие/полнота обхода с раздельными датами (A06); устойчивый журнал `PendingChange` и надёжные уведомления/email + UNIQUE-ключ уведомления (A16); безопасность XML/HTTP — выделенный SSRF-клиент и защита XXE (A13/A14); языковая и зарплатная честность (A07/A09); приватность с проверкой на **втором** пользователе (A23); наблюдаемость свежести/ошибок и базовое восстановление БД/снимков (A29). Сканирование образов и защита реальных персональных данных — до любого публичного пилота.

  *Остальное Этапа 1:* авторизация (сессия + cookie); сохранение вакансий с причиной скрытия; поиск и фильтры (зарплата/язык) с курсорной пагинацией; SPA с trust-панелью и онбординг (после ядра); контейнерная сборка (`bootBuildImage`) + Compose; базовый CI (компиляция, тесты) и интеграционные тесты (Testcontainers); заглушка источника.
- **Зависимости.** Этап 0.
- **Критерий готовности** — заменён критерием «ядро чётко работает» (план развития, §3; ADR-20).

### Этап 2 — Полезный и надёжный мониторинг

- **Состав.** (Брокер, outbox и publisher confirms введены на Этапе 1 — ADR-10; здесь конвейер только расширяется.) Задания ограниченного размера, повторы и DLQ; несколько адаптеров систем найма с общим контрактом (включая энтерпрайз-вендоров сверх первого трека Этапа 1); расширение автоматического обнаружения (доп. входы по плану развития) и ручной ввод компании клиентом (третий источник, план развития E4); **обязательное минимальное нечёткое сопоставление публикаций разных источников** (§6, ADR-14); технологический профиль компании (агрегация ядро/контекст, объяснимость, объём выборки, оверлей подготовки под вакансию); сравнение сохранённых вакансий; «откликнись напрямую» для дублей; статус «вероятно закрыта» (после соответствующего состояния закрытия); расширенные интеграционные тесты (Testcontainers) и повторная обработка снимка; калиброванное сопоставление на размеченной выборке (§13, A28).
...
Расширение обнаружения по связям (портфели, каталоги); внешнее обогащение техпрофиля и LLM-формулировки талкинг-поинтов с проверкой; дополнительные системы найма и новые рынки; полный профиль кандидата и файлы CV; полное текстовое нечёткое сопоставление и аналитика; геопоиск; дополнительные каналы уведомлений и календарь; операторы БД/брокера и PITR; полная GDPR-машинерия при публичном использовании; scale-to-zero с изменённой архитектурой планирования. Расширения не являются условием завершения основной версии. Собственные карьерные страницы без разметки `JobPosting` — вне продукта (ADR-19, ADR-20).
...
**ADR-6. Обнаружение с гейтом уверенности.** Автоматически найденный источник может быть ошибочным. Выбор — уверенный резолв подключается автоматически, неуверенный уходит на подтверждение человеку. Обоснование — рост базы без ручного труда, но без мусорных источников. Последствие — очередь подтверждения и отдельный бюджет обнаружения; критический контур SSRF (§9). **Пересмотр 2026-09-25 (ADR-20):** очередь подтверждения человеку отменена — неуверенное пропускается или перепроверяется автоматически; гейт уверенности и проверка принадлежности остаются. Построенная очередь (`/api/v1/admin/employer-candidates`) заменяется автоматическим подключением (план развития, K4).
...
**ADR-14. Нечёткое сопоставление источников: граница между документами (A02).** Ценность «и там, и там» (§7.1 бизнес-ТЗ) держится на связывании объявления площадки и ленты ATS, а точные признаки его почти не дают. Согласованная граница: **в пилоте — только точное сопоставление плюс честное состояние покрытия «не проверено/неизвестно»** (ADR-15); если на выбранной паре источников этого мало, условием пилота становится минимальное сопоставление или ручная проверка ограниченной выборки. **На Этапе 2 — обязательное минимальное нечёткое сопоставление** (работодатель + нормализованный заголовок + близость дат, с порогом; сомнительное — на ручную проверку). Полное текстовое сопоставление — расширение (в бизнес-ТЗ «нечёткое объединение дублей» переименовано в него). Обоснование — не обещать в пилоте того, что даёт только Этап 2. Последствие — precision/recall связывания измеряются на размеченных парах (§13); метка без проверенного основания не считается пройденной приёмкой (§7.8). **Пересмотр 2026-09-25 (ADR-20):** покрытие исключено — связывание площадки и ленты ATS для метки больше не нужно; минимальное нечёткое сопоставление остаётся только для дублей между источниками одной компании (лента системы найма и собственная страница); сомнительные пары остаются отдельными записями, ручной проверки нет.

**ADR-15. Покрытие — результат сравнения, а не факт (A01). Отменено 2026-09-25 (ADR-20).** «Скрытая» уверенно недоказуема при недоступной площадке, неполном обходе или неоднозначном сопоставлении. Выбор — отдельная сущность `CoverageAssessment` с состоянием, включающим **«не проверено/неизвестно»**, областью и полнотой проверки, набором проверенных площадок, версией сопоставления и датой; отрицательный результат формулируется ограниченно («не найдена на X, Y при [дате]»). Отклонено вычисление флага только по наличию/отсутствию публикаций (сливает «нет проверки» и «нет на площадке»). Последствие — метка «скрытая» проверяема, пересчитывается при смене набора площадок. **Отмена (ADR-20):** признак покрытия исключён из продукта; `CoverageAssessment` (V29) и связанный код удаляются отдельным срезом.

**ADR-16. Модель источников: провайдер ≠ источник ≠ компания (A03).** ER, где `Source` принадлежит одной `Company`, не описывает площадку с тысячами работодателей и кандидата до создания компании. Выбор — `Provider` (тип интеграции + capability-карточка), `Source` (доска/область сбора), `CompanySource` (проверенная связь источника с работодателем, для площадки многие-ко-многим), `EmployerCandidate` с необязательной связью с `Company`. Лимиты действуют на провайдера/credential/хост/доску. Последствие — одна площадка отдаёт вакансии многих компаний, одна компания имеет несколько досок, смена домена не создаёт второго работодателя. **Уточнение 2026-09-25 (ADR-20):** площадки как источник вакансий не используются (только как источник названий компаний); `Source` — доска системы найма или карьерная страница компании.

**ADR-17. Охват — по вендор-адаптерам ATS, включая энтерпрайз (расширение ADR-5, ревизия 2026-09-22).** Контекст — главная ценность продукта — вакансии, которых нет на агрегаторах, а у крупных работодателей (напр. Swiss Re, Raiffeisen Bank International — SAP SuccessFactors; часть Deutsche Telekom — SmartRecruiters/кастом) карьерные сайты стоят на энтерпрайз-ATS, а не на SMB-платформах из исходного списка §5. Ограничение охвата только SMB-вендорами оставило бы самые ценные «скрытые» ленты вне продукта. Выбор — охват определяется **наличием вендорного шаблона адреса ленты**, а не размером компании: в охват входят и SMB-вендоры (Greenhouse, Lever, Ashby, Workable, Recruitee, Personio, SmartRecruiters), и энтерпрайз (Workday — де-факто открытый `/wday/cxs/.../jobs`; SAP SuccessFactors — XML-фид/OData/публичные тенанты). Каждый вендор — один адаптер за общим контрактом; обнаружение идёт по хостовому шаблону вендора (CT/Common Crawl, §5); порядок ввода вендоров — по «ценность × достижимость» (ориентир первым — Workday). Отклонено — универсальный ридер произвольных кастомных страниц без вендорного шаблона (дорого, хрупко, без гарантий): это единственный оставшийся non-goal сбора. Последствие — доступность ленты и правовое основание энтерпрайз-вендоров проверяются **по-тенантно** (A24); часть SuccessFactors-витрин требует рендеринга страницы (отдельный, более дорогой контур); контракт адаптера остаётся общим для SMB и энтерпрайз. **Уточнение 2026-09-25 (ADR-19, ADR-20):** ценность — вакансии с карьерных страниц компаний региона, а не «скрытые» ленты; граница охвата — «вендорный шаблон **или** разметка `JobPosting`».

**ADR-18. Отличие продукта — курирование, честность покрытия и мониторинг, а не эксклюзивный доступ к лентам (ревизия 2026-09-22).** Контекст — ленты ATS (включая Workday и SuccessFactors) читаемы не только нами: их массово собирают агрегаторы и готовые скрейперы; «скрытость» вакансии **не следует** из «мы умеем читать эту ленту». Позиционирование на «эксклюзивном доступе» было бы ложной посылкой. Выбор — моат строится на: (1) **широте курируемого охвата** — длинный хвост карьерных досок в нише, включая те, что конкретный сравниваемый агрегатор не проиндексировал; (2) **честности покрытия и свежести** — метка «скрытая» = «не найдена на [площадках] при проверке [дата]», с состоянием «не проверено/неизвестно» (ADR-15); (3) **мониторинге во времени** (подписки на компании, «было → стало»); (4) **техпрофиле компании** и подготовке под вакансию; (5) **чистом сфокусированном интерфейсе**. Отклонено — заявление об уникальном доступе к данным. Последствие — ценность измеряется покрытием и честностью в ограниченной **проверенной области сравнения** (паспорт пилота, §13/A28), а не претензией на эксклюзивный доступ; это уточняет, но не отменяет бизнес-ТЗ §5. **Пересмотр 2026-09-25 (ADR-20):** пункт (2) сужен до честности данных и свежести — метки «скрытая» и сравнения с площадками нет; ценность измеряется отчётом ядра (доля найденных сайтов и карьерных страниц, живые вакансии ниши), а не покрытием.

**ADR-19. Обнаружение от региона и профиля; собственные страницы со schema.org `JobPosting` (ревизия 2026-09-25, решение владельца; «профиль» отменён ADR-20).** Контекст — цель продукта: вакансии с карьерных страниц компаний **заданных регионов и профиля**; площадки — во вторую очередь, ручной ввод — дополнение. Вход «от вендора» (все доски Workday/Personio в индексе Common Crawl с отсевом по рынку) даёт в основном чужой рынок (стенд project-notes §91: 27 из 42 досок Personio вне рынка; Workday — глобальные корпорации) и видит региональную компанию лишь если она случайно на этих вендорах. Выбор — основной вход идёт **от компаний региона с профилем** (открытые реестры юрлиц с кодом деятельности, открытые каталоги, ручной ввод) к их сайту и карьерной странице; тип страницы определяется на месте: вендор — адаптер вендора (ADR-5, ADR-17), собственная страница — общий адаптер по стандартной разметке schema.org `JobPosting` (структурированные данные, которые сайт публикует сам). Принадлежность сайта компании подтверждается обязательными реквизитами (IČO / номер Firmenbuch в выходных данных). Вендорный гарвест остаётся дополнительным входом. Отклонено — разбор произвольной разметки собственных страниц без `JobPosting` (дорого, хрупко). Последствие — уточняет ADR-5/ADR-17: граница охвата — «вендорный шаблон **или** разметка `JobPosting`»; бизнес-ТЗ §7.1 обновлено; правовые основания реестров и каталогов — в реестре `current-state.md`. **Уточнение 2026-09-25 (ADR-20):** «профиль компании» и код деятельности как фильтр отменены — компании региона берутся без фильтра по виду деятельности и размеру, ниша определяется по вакансии; ручной ввод — не часть основного входа, а третий источник по желанию клиента.

**ADR-20. Сначала ядро; всё автоматически; без признака «скрытая»; ручной ввод — третий источник (ревизия 2026-09-25, решение владельца, project-notes §97–§98).** Контекст — разработка уходила в «глянец» (фильтры, подписки, покрытие), а ядро — найти компании региона, их карьерные страницы и вакансии — не работало. Выбор: (1) порядок работ — план развития `docs/roleorienta-plan-korrektirovka.md`: сначала ядро K1–K8 с измерителями, всё остальное — только после критерия «ядро чётко работает» (план, §3); построенное заранее заморожено; (2) вакансия существует, только если она есть на карьерной странице компании; нет страницы, пригодной для структурного чтения, — компания пропускается; размер и профиль компании — не критерии; (3) обнаружение полностью автоматическое: неуверенное пропускается или перепроверяется, очереди подтверждения человеку нет; (4) источники компаний по порядку — открытые реестры юрлиц и индексы систем найма, затем площадки-агрегаторы как источник названий компаний (только с разрешением; их вакансии не собираются), затем ручной ввод компании (опция по желанию клиента; тот же автоматический путь); (5) признак покрытия и метка «скрытая» исключены. Отклонено — сравнение с площадками как основа ценности: требует читать площадки, чьи условия это запрещают (project-notes §90), и не помогает находить вакансии. Последствие — отменяет ADR-15; пересматривает ADR-6, ADR-14, ADR-18; уточняет ADR-16, ADR-17, ADR-19; код покрытия (V29, пакет `coverage`, параметр и поле `coverage` в API ленты, строка покрытия в дайджесте) и очередь подтверждения удаляются/заменяются отдельными срезами; изменение API — с разрешения владельца.
