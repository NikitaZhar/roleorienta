...
Дата: 2026-09-11
Статус: надо прочитать весь документ
---
...
| **Система найма** (ATS) | Готовая платформа карьерных страниц (Greenhouse, Lever, Ashby, Workable, Recruitee, Personio и т.п.), часто отдающая список вакансий компании открыто |
...
| `PendingChange` | Устойчивый журнал изменений, ожидающих сопоставления с подписками. Работает **с пилота** (без очереди): гарантирует, что уведомление не теряется при падении между записью ревизии и рассылкой (§8) |
| `OutboxEvent` | Событие для надёжной публикации (используется с Этапа 2, когда вводится очередь; в пилоте не задействовано) |
...
Плюс адаптер(ы) мейнстрим-площадок региона. Единый и предсказуемый формат адреса лент — это ещё и то, что делает автоматическое обнаружение посильным (адрес выводится по шаблону из домена и типа системы).
...
- **Гарвест ATS-хостов из публичных датасетов — основной вход.** Common Crawl и Certificate Transparency дают **разные типы кандидатов и не взаимозаменяемы (A11):**
  - **Индекс URL Common Crawl** — для **path-based** ATS (`boards.greenhouse.io/<slug>`, `jobs.lever.co/<slug>`), где slug стоит в пути. Индекс исторический и не гарантирует все живые доски; массовую фильтрацию вести по колоночному индексу, не перегружая CDX-сервис.
  - **Логи Certificate Transparency** хранят **имена хостов, а не URL-пути**, поэтому годятся только для **поддоменных** ATS (`<slug>.recruitee.com`, `<slug>.jobs.personio.de`); slug из `greenhouse/lever` они не дают, а wildcard-сертификат не перечисляет все реально существующие поддомены.
...
Любой вход даёт лишь *кандидата*: каждый проходит резолв системы найма, валидацию ленты, гейт уверенности и SSRF-защиту (§9), а также **дедупликацию работодателей** (одна компания не заводится дважды из разных входов). На пилоте достаточно одного основного входа (гарвест ATS-хостов по одному-двум шаблонам); остальные входы наращиваются на Этапе 2 (§14).
...
**Шаги конвейера (Этап 2, с очередью).**
...
**Пилотное исполнение (Этап 1).** Те же роли и стадии выполняются последовательным периодическим планировщиком **без RabbitMQ и outbox**; обнаружение и агрегация — периодические задания. Переход на очередь (Этап 2) не меняет модель данных и контракты — только механизм доставки.
...
- **Условия использования** и лимиты источников соблюдаются; сбор — только с открытых лент и разрешённых страниц. Разрешение локальных заглушек — только в тестовом профиле.
...
- **Состав.** Выбрать рынок; проверить доступ к одной площадке и одной системе найма (примеры полей, включая зарплату и языки, лимиты и условия использования); проверить, на каких системах сидят интересующие компании; предзаготовить курируемый список работодателей; прототип одного способа автоматического обнаружения; заглушка источника с нормальными и ошибочными ответами.
- **Зависимости.** Стартовая точка.
- **Критерий готовности.** Подтверждена возможность получить нужные данные (включая вакансии с сайтов компаний) и работоспособность автообнаружения в ограниченном виде.

### Этап 1 — Рабочий продукт в Compose (пилот)

- **Состав.** Monorepo, два Spring Boot приложения, PostgreSQL + Flyway с начала; **RabbitMQ + Spring AMQP + transactional outbox с самого начала** (доставка заданий at-least-once, publisher confirms, идемпотентные обработчики; планировщик под leader-lock — ADR-1/ADR-10/ADR-12); один адаптер площадки + один адаптер системы найма за общим контрактом; нормализация основных полей (зарплата, языки); точная дедупликация со связыванием «площадка + сайт компании» и признак «скрытой»; история изменений с первого сбора; структурированные требования (извлечение по таксономии навыков и правилам, тристейт языков — §6) + профиль навыков и явное сопоставление; подписки на компании; поиск и фильтры (покрытие/зарплата/язык) с курсорной пагинацией; авторизация (сессия + cookie); сохранение вакансий с причиной скрытия; отклики и заметки; дайджест изменений + email; «сообщить об ошибке»; SPA с trust-панелью; онбординг с непустым первым экраном; автоматическое обнаружение в ограниченном виде — один основной вход (гарвест ATS-хостов, §5) с гейтом уверенности — наряду с ручным; контейнерная сборка (`bootBuildImage`) + Compose; базовый CI (компиляция, тесты) и базовые интеграционные тесты (Testcontainers); заглушка источника. **Обязательно уже в пилоте (по итогам аудита):** состояние покрытия «не проверено/неизвестно» и ограниченные формулировки (A01); закрытие/полнота обхода с раздельными датами (A06); устойчивый журнал `PendingChange` и надёжные уведомления/email (A16); безопасность XML/HTTP — выделенный SSRF-клиент и защита XXE (A13/A14); языковая и зарплатная честность (A07/A09); приватность с проверкой на **втором** пользователе (A23); наблюдаемость свежести/ошибок и базовое восстановление БД/снимков (A29). Сканирование образов и защита реальных персональных данных — до любого публичного пилота.
- **Зависимости.** Этап 0.
- **Критерий готовности.** Законченный пользовательский путь; видны вакансии, которых нет на площадке; работодатели добавляются автоматически; история и технологии видимы с первого дня. Пользователь находит интересное, чего не видел на площадках, и хочет получать обновления.

### Этап 2 — Полезный и надёжный мониторинг

- **Состав.** (Брокер, outbox и publisher confirms введены на Этапе 1 — ADR-10; здесь конвейер только расширяется.) Задания ограниченного размера, повторы и DLQ; несколько адаптеров систем найма с общим контрактом; расширение автоматического обнаружения (доп. входы: резолв по домену, связи) и предложения компаний пользователями; **обязательное минимальное нечёткое сопоставление публикаций разных источников** (§6, ADR-14); технологический профиль компании (агрегация ядро/контекст, объяснимость, объём выборки, оверлей подготовки под вакансию); сравнение сохранённых вакансий; «откликнись напрямую» для дублей; статус «вероятно закрыта» (после соответствующего состояния закрытия); расширенные интеграционные тесты (Testcontainers) и повторная обработка снимка; калиброванное сопоставление на размеченной выборке (§13, A28).
...
**ADR-5. Адаптер на систему найма, а не универсальный парсер.** Карьерные страницы работают на распространённых системах найма. Выбор — один адаптер за общим контрактом на каждую систему. Обоснование — переводит задачу из невозможного «парсить любой сайт» в посильное; даёт полную доску компании для техпрофиля. Последствие — честная граница охвата (компании вне распространённых систем не гарантированы).
