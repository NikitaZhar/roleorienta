...
Дата: 2026-09-22
Статус: автоматический вход обнаружения Workday по Common Crawl (§55, A1b-2) — V26 (harvest_cursor + harvested_board), HarvestStore (аренда вместо удержания транзакции, атомарная фиксация страницы, дедуп без учёта регистра), CcHarvestScheduler (сбор под арендой вне транзакции → fan-out DISCOVER_EMPLOYER под leader-lock 1004), устойчивый разбор CDX (обрыв → страница не засчитана) и мелкие страницы pageSize=1 под тайм-аут шлюза индекса; CcHarvestTrigger выключен по умолчанию до B1/B2. Дальше: B1 (rate-limit/Retry-After), затем включение и A2. Компиляция/тесты в среде ИИ не запускались — их выполняет владелец.
Прежний статус: вход Workday — Common Crawl вместо CT, CommonCrawlIndexClient и WorkdayBoard.fromCareerUrl (§54); Workday site-резолв по словарю через cxs (§53, A1b-1); CT-клиент CertSpotterClient (§52, A1a); Workday сквозь обнаружение end-to-end на заглушке (§51); первый энтерпрайз-адаптер Workday (§50); протокол Этапа 0 (§49); ревизия документации — вендор-инклюзивный охват и ADR-17/18 (§48); интеграция персональных маркеров в ленту (§31); персональные маркеры (§30); аутентификация (§29); REST-чтение и фильтры (§26–28); извлечение по таксономии/опыт/stance (§20–25); нормализация зарплаты/локации и история (§17–19); конвейер сбора (§13–16)
---
