...
Дата: 2026-09-22
Статус: ревизия документации (§48) — охват расширен на вендор-адаптеры, включая энтерпрайз-ATS (Workday/SuccessFactors/SmartRecruiters); отличие продукта переформулировано как курирование/честность покрытия/мониторинг, а не эксклюзивный доступ к лентам; §4/§6 техдока синхронизированы с ADR-10 (брокер+outbox с Этапа 1); добавлены ADR-17 (охват) и ADR-18 (отличие); §14 переприоритизирован (сначала механизм автообнаружения); правки в business-requirements и README; добавлена docs/roleorienta-plan-korrektirovka.md (выверенный по коду разбор + бэклог дефектов). Кода изменение не касается.
Прежний статус: интеграция персональных маркеров в ленту (§31); персональные маркеры публикации (§30); модуль аутентификации (§29); REST-чтение и фильтры (§26–28); извлечение по таксономии/опыт/stance (§20–25); нормализация зарплаты/локации и история изменений (§17–19); конвейер сбора (§13–16)
---
