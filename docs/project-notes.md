...
## §69 — Сеньорность: заголовок главнее описания

Пункт 4 из §64.2. У вакансий Workday уровень почти всегда `UNKNOWN`, хотя он есть в заголовке
(«Senior Technical Architect», «Junior Electrical Designer»): `ExperienceExtractor` смотрел только
в описание, а там уровни встречаются в постороннем смысле («senior management», «junior
colleagues») — правило «ровно один уровень в тексте» давало `UNKNOWN`.

### 69.1 Код

- `ExperienceExtractor.extract(title, description)`: уровень — из заголовка (то же правило:
  ровно один уровень), если в заголовке его нет или он неоднозначен («Junior/Senior») — из
  описания; число лет — из описания, как раньше. Прежний `extract(description)` оставлен и
  делегирует с `title = null` (его используют тесты).
- `PostingEnricher` передаёт `posting.getRawTitle()`.

### 69.2 Что проверяют тесты

`ExperienceExtractorTest`: заголовок главнее описания с посторонними «senior/junior»; без уровня в
заголовке — описание; неоднозначный заголовок — описание; заголовок без описания. Прежние
тесты не изменены. README по §0.2 не затронут.

### 69.3 Что НЕ вошло

Слова «Lead / Principal / Staff / Sr. / Jr.» как уровень — не добавлены (неоднозначны: «Product
Owner / Lead»); уже собранные публикации получат уровень при следующей детали.

## §70 — Лента «свежие первыми»; уровень только из заголовка

### 70.1 Сортировка ленты по дате публикации (job-api)

- Параметр `sort=POSTED` (по умолчанию `ID` — прежний порядок, ничего не меняется): по убыванию
  `posted_on`, публикации без даты — в конце, при равной дате — по убыванию `id`.
- `FeedPaging(cursor, limit, sort)` — параметры страницы одной записью (как `PostingFilter`):
  иначе у `PostingQueryService.list` и контроллера стало бы 6 параметров (лимит §3.10).
- `PostingReadRepository.searchByPosted` — пагинация по ключу (keyset) «дата + id»: страница
  начинается строго после ключа последней публикации предыдущей. Отсутствующая дата в ключе
  заменяется `NO_DATE = 1900-01-01` (`coalesce`), поэтому публикации без даты идут последними
  и листаются тем же условием. `NO_DATE` передаётся в запрос методом записи
  (`:#{#after.noDate()}`): значение не `null` и стоит рядом с колонкой даты — тип определяется
  (см. §68 о типе параметра-даты).
- Фильтры обеих лент — одна JPQL-константа `PostingReadRepository.FILTERS`, чтобы не разошлись.
- Курсор — по-прежнему одно число (`nextCursor` в JSON не меняется): `PostedKeyset.toCursor()`
  упаковывает номер дня (22 бита, со сдвигом) и `id` (31 бит). Всего ≤ 53 бит — JavaScript-клиент
  читает число из JSON без потери точности; предел `id` — ~2,1 млрд публикаций.

```mermaid
flowchart LR
  Q["GET /postings?sort=POSTED&cursor=N"] --> K["PostedKeyset.fromCursor(N)<br/>(дата, id)"]
  K --> R["searchByPosted: coalesce(posted_on, 1900-01-01), id<br/>строго меньше ключа, по убыванию"]
  R --> P["страница + nextCursor = PostedKeyset.of(последняя).toCursor()"]
```

### 70.2 Уровень только из заголовка (job-worker)

Стенд §69: уровень, взятый из описания при отсутствии его в заголовке, был ложным у ~9 вакансий
(«Contracts Analyst 1», «Attorney 1» → SENIOR: в описаниях «senior management»). Теперь
`ExperienceExtractor.extract(title, description)`: уровень — только из заголовка, иначе
`UNKNOWN`; число лет — из описания. Однопараметровый `extract(description)` удалён.

### 70.3 Что проверяют тесты

`PostingApiIntegrationTest.sortPostedNewestFirstUndatedLastAndPaginates` — A (с датой) первой,
C и B без даты — в конце, вторая страница по `nextCursor`. `PostingQueryServiceTest.
postedCursorRoundTripIncludingUndated` — курсор туда-обратно, для даты и без неё, ≤ 2^53; прочие
тесты сервиса — на `FeedPaging`. `ExperienceExtractorTest` — уровень только из заголовка
(описание с «senior management» → `UNKNOWN`, неоднозначный заголовок → `UNKNOWN`). README по
§0.2 не затронут.

### 70.4 Стенд

`target/stand-feed.sh` дополнен: `sort=POSTED` и вторая страница по `nextCursor`.

### 70.5 Что НЕ вошло

Индекс под `coalesce(posted_on, …) desc, id desc` — на объёме пилота не нужен (план запроса —
при росте ленты).

## §71 — Потолок размера тела ответа источника (B2)

Пункт B2 плана (`roleorienta-plan-korrektirovka.md`): вместе с B1 (темп, §57) — условие
включения автоматического входа Common Crawl (A1). До этого `SourceHttpClient` читал тело
целиком (`retrieve().body(String.class)`): огромный или «бесконечный» ответ источника занимал
память и поток до тайм-аута (отложено в §32.6).

### 71.1 Код

- `SourceHttpClient`: все три запроса (`getBody`, `getJson`, `postJson`) читают ответ через
  **`exchange`** — метод `RestClient`, который отдаёт сам ответ (статус, заголовки, поток тела)
  и позволяет читать тело вручную
  (https://docs.spring.io/spring-framework/reference/integration/rest-clients.html#_exchange).
  Тело читается потоком не больше `max-body-bytes` байт (`InputStream.readNBytes(max + 1)`);
  больше — `ResponseTooLargeException` (вложенный класс, `IllegalStateException`), остаток не
  читается и в память не попадает.
- Неуспешный статус обрабатывает **`DefaultResponseErrorHandler`** — тот же обработчик, что у
  `retrieve()`: исключения прежние (`HttpClientErrorException.TooManyRequests`, 404 у Common Crawl
  и т.п.), логика `Retry-After` (§57) не меняется.
- Кодировка — из `Content-Type`, без неё — UTF-8 (для JSON так было и раньше).
- `SourceHttpProperties` (`app.collect.http`: `connect-timeout-ms`, `read-timeout-ms`,
  `max-redirects`, `max-body-bytes`) — настройки клиента одной записью вместо трёх `@Value`:
  иначе у конструктора было бы 7 параметров (лимит §3.10; было 6 — нарушение устранено).
  Значения по умолчанию прежние, `max-body-bytes` — 5 МБ (ленты Workday — десятки КБ, страница
  Common Crawl при `page-size: 1` — ~0,5 МБ).
- `application.yml`: комментарий к `app.discovery.cc.enabled` — условия B1/B2 выполнены, включение
  входа — отдельным решением (A1). План: B2 отмечен выполненным.

### 71.2 Что проверяют тесты

`SourceHttpClientFetchTest.bodyAtLimitIsReadAndLargerIsRejected` — тело ровно на пределе
читается, на байт больше — `ResponseTooLargeException`. Прежние тесты клиента
(`SourceHttpClientRetryAfterTest` — 429/503 и `Retry-After`, `SourceHttpClientSsrfTest`,
`followsRedirect`) и адаптеров/Common Crawl проверяют, что поведение статусов и тел не
изменилось; `SourceHttpClientRetryAfterTest` переведён на конструктор с `SourceHttpProperties`
(прежний 6-параметровый удалён). README по §0.2 не затронут.

### 71.3 Что НЕ вошло

Включение автоматического входа Common Crawl (A1) — следующий шаг, решение владельца.

## §72 — A1: автоматический вход Common Crawl включён по умолчанию

Пункт A1 плана — ядро гипотезы: приложение само находит работодателей и их карьерные ленты.
Вход построен в §55 (`CcHarvestScheduler`) и был выключен до B1 (темп и `Retry-After`, §57) и
B2 (потолок размера ответа, §71). Оба условия выполнены.

### 72.1 Изменения

- `application.yml`: `app.discovery.cc.enabled: true`. Бюджет прежний и малый: тик раз в 10 мин,
  1 страница индекса (~3 тыс. URL) и не более 20 заданий `DISCOVER_EMPLOYER` за проход; темп
  к `commoncrawl.org` — 3 с, к `myworkdayjobs.com` — 2 с (§57). Цепочка: индекс → доски
  (`harvested_board`, дедуп) → гейт рынка SK/AT (§56–§59) → `HIGH` — подключение источника →
  сбор ленты рынка (деталь — ниша и 30 запросов в сутки на источник, §63).
- `JobWorkerApplicationTests`: `app.discovery.cc.enabled=false` — тест загрузки контекста
  работает без профиля `test` и не должен ходить в интернет (остальные интеграционные тесты
  выключают обход через `application-test.properties`).
- README (§0.2 — статус проекта): вход включён; сводка «построено / в работе» приведена к
  фактическому состоянию (сбор Workday по рынку, потолок ответа, лента с фильтрами).
- План: статус A1.

### 72.2 Стенд

`target/stand-cc.sh` (вне git): воркер с тиком CC раз в 3 мин, 15 мин (`MIN=`); отчёт
`target/stand-cc.txt` — курсор индекса, доски за прогон (новые/уже известные), итог гейта,
подключённые работодатели с числом публикаций на рынке, кандидаты на ручную проверку,
собранные публикации новых источников по стране, лог обхода, ошибки. В конце новые источники
ставятся на паузу (гигиена стенда). Результат прогона — отдельной записью.

### 72.3 Что НЕ вошло

Проверка принадлежности доски работодателю (A2) и дедуп работодателей между входами (A3) — по
итогам прогона.

## §73 — Обход Common Crawl: очередь не растёт, гейт без фасетов, локаль `en-us`

Три дефекта первого прогона A1 (стенд §72: 5 страниц индекса, 755 досок, 100 проверено,
3 работодателя подключены, ошибок нет).

### 73.1 Очередь досок не растёт без предела

Страница индекса даёт ~150 досок, fan-out за проход — 20: после 5 проходов в очереди `NEW`
осталось 655, и с каждым проходом росло на ~130. `CcHarvestScheduler.collect()` теперь не читает
новую страницу (и не берёт аренду), пока в очереди не меньше `max-fan-out` досок
(`HarvestStore.countNewBoards`); проход только передаёт накопленное. Тест
`CcHarvestSchedulerTest.backlogStopsReadingNewPages`.

### 73.2 Гейт рынка без фасетов — по локациям вакансий первой страницы

23 из 34 кандидатов «на ручную проверку» — тенанты США (университеты `tamus/*`, `upenn` …), у
которых Workday не отдаёт ни фасета стран, ни фасета локаций: гейт писал «рынок не проверен» и
ставил `PENDING`. Теперь `DiscoveredPosting` несёт `rawLocation` — локацию из строки списка
(Workday `locationsText`; 3-аргументный конструктор сохранён, Greenhouse не затронут), и
`DiscoverEmployerJobHandler` в этом случае проверяет локации вакансий первой страницы тем же
правилом, что и фасет локаций (§59, §61): рыночная → `HIGH`, только неоднозначные → ручная
проверка, ни одной → `OUT_OF_MARKET`. Причина начинается с «по вакансиям первой страницы
(фасетов нет)» — это выборка (до 20 вакансий), не вся лента. Сводки «2 Locations» страны не
называют и пропускаются; если других локаций нет — как раньше, «рынок не проверен». Тесты
`DiscoverEmployerJobHandlerTest.withoutFacetsTenantIsJudgedByPostingLocationsOfFirstPage`,
`onlyLocationSummariesLeaveMarketUnchecked`; `WorkdayAdapterTest.listParsesPostingAndFields` —
`rawLocation`.

### 73.3 Локаль `en-us` не принимается за имя сайта

4 доски получили сайт `en-us` (POST → 404): разборщик адреса узнавал локаль только как `en-US`.
`WorkdayBoard.LOCALE` — регион в любом регистре (`^[a-z]{2}(-[A-Za-z]{2})?$`); двухбуквенный сайт
в верхнем регистре (`american/AU`) по-прежнему не локаль. Тест
`WorkdayBoardTest.lowercaseRegionLocaleIsNotASite`.

### 73.4 Стенд и что НЕ вошло

Проверка — тем же `target/stand-cc.sh`. Уже проверенные кандидаты не перепроверяются (4 доски
`en-us` и «рынок не проверен» из §72 остаются как есть). Не вошло: сбор с подключённого по
выборке тенанта без фасетов идёт без фильтра рынка на стороне источника (фильтровать нечем);
README по §0.2 не затронут.

## §74 — Недоступная доска — `UNREACHABLE`, а не ручная проверка

Стенд §73: 29 из 32 кандидатов на ручной проверке — `NONE`, лента не открылась: 422 (14 досок —
vhr-otsuka ×7, workiva, zelis …), 404/403 (9 — wmeimg, wework, vsp …), 404 у досок с именем-локалью
`en-us` (6 — попали в очередь до §73). Проверка в браузере: страница тенанта workiva уводит на
community.workday.com — тенанта больше нет; Workday отвечает 422 на несуществующий сайт/тенант.
Адреса из индекса Common Crawl устаревают — человеку в таких случаях проверять нечего.

### 74.1 Код

- `EmployerCandidateState.UNREACHABLE` (core) — лента по адресу доски окончательно не
  открывается. Столбец `employer_candidate.state` — `TEXT` без CHECK (V18): миграция не нужна.
- `DiscoverEmployerJobHandler`: ответ `403`/`404`/`410`/`422` на проверку ленты
  (`isUnreachable`) → `NONE` и `UNREACHABLE`, причина «лента недоступна: …»; прочие ошибки
  (400/401, неразборный ответ) — как раньше, `NONE` на ручную проверку; временные сбои (429,
  5xx, сеть) по-прежнему повторяются (§57). Результат проверки (`Assessment`) несёт итоговое
  состояние кандидата вместо флага `outOfMarket` — ветвление `handle()` стало одним путём:
  `CONFIRMED` — подключение, иначе — сохранить с состоянием проверки.
- Доски с именем-локалью, ещё стоящие в очереди с §72, отдельной чистки не требуют: при проверке
  они получают 404 и уходят в `UNREACHABLE`.

### 74.2 Что проверяют тесты

`DiscoverEmployerJobHandlerTest.goneOrForbiddenBoardIsUnreachableNotQueued` (422 → `UNREACHABLE`,
без подключения), `unreachableClassification` (403/404/410/422 — да; 400 и ошибка разбора —
нет); прежние тесты гейта (HIGH, вне рынка, по локациям, неразборная лента → `PENDING`,
временные сбои) не изменились. README по §0.2 не затронут.

### 74.3 Стенд

`target/stand-cc.sh`; кандидаты, проверенные до §74, переводятся в `UNREACHABLE` однократным
SQL на стенде (команда — в ответе владельцу), чтобы очередь ручной проверки отражала новое правило.

## §75 — Регламент сессии и документ текущего состояния

Изменение — **только документация**. По решению владельца правила работы ассистента, которые
раньше держались на напоминаниях, собраны в обязательный документ; причина — неправильный старт
сессии 2026-09-24 (контракт урезан и не прочитан, патч вместо правки файлов, расширение задачи
без разрешения, рассуждения вместо команд, сломанная сигнатурой компиляция теста, безусловные
команды прогона).

- `docs/session-protocol.md` — регламент: старт сессии (проверка целостности контракта §0–§7,
  чтение регламента и `current-state.md`, какая версия кода исходная — рабочая копия владельца
  может опережать GitHub, проверка счётчика аудита), ведение среза (поиск всех вызовов при
  смене сигнатуры, типы nullable-параметров в JPQL, проверки, доступные ассистенту), сдача
  (файлы прямо в рабочую копию, формат §0.5, команды с условиями, `target/verify.log`),
  завершение (project-notes, `current-state.md`, счётчик, удаление облачной копии), **срез-аудит
  каждые 10 срезов** (чек-лист из 6 пунктов, запись `§NN — Аудит`, сброс счётчика),
  **автоматические проверки в CI** (цель: Checkstyle + ArchUnit, пункт C4 плана).
- `docs/current-state.md` — «как устроено сейчас» на 2–3 страницы: назначение, стек и запуск,
  модули, путь данных, соглашения, стенды, статус плана, **счётчик срезов с последнего аудита**
  (сейчас 10: учёт с §66 — следующий срез аудит).
- `docs/working-contract.md` — §0.6: регламент и документ состояния обязательны.
- `docs/README.md` — ссылки на оба документа. README корня по §0.2 не затронут.

## §76 — Аудит (срезы §66–§75)

Первый срез-аудит по регламенту сессии §5 (счётчик достиг 10). Область — код и документы,
изменённые в §66–§75 (34 файла: зарплата из текста, лента, уровень, потолок ответа, вход Common
Crawl, гейт, регламент). Новой функциональности нет.

### 76.1 Найдено и исправлено

1. **Логика в репозитории (контракт §3.3).** Ключ пагинации «дата + id» с упаковкой в курсор
   (`PostedKeyset`, §70) был вложенной записью `PostingReadRepository`. Вынесен в отдельный файл
   `api/posting/PostedKeyset.java`; в репозитории остались только запросы. Поведение не меняется.
2. **Полные имена классов вместо импортов** в `DiscoverEmployerJobHandler`
   (`java.util.Set/List/LinkedHashMap` в коде) — заменены импортами, как в остальном проекте.
3. **Устаревший JavaDoc** `WorkdayAdapter.parseDetail`: зарплату из текста берёт
   `SalaryNormalizer.resolve` (с §66, после выноса в `normalize`), а не `PostingEnricher` напрямую.
4. **Настройки HTTP-клиента не в `application.yml`** (соглашение `current-state.md`: значения и
   комментарии — в конфигурации): добавлен блок `app.collect.http` (тайм-ауты, редиректы,
   `max-body-bytes`) с теми же значениями по умолчанию.

### 76.2 Проверено, нарушений нет

Параметры ≤5 у методов и конструкторов изменённых классов; однобуквенных имён и чисел без
констант нет (`SalaryTextExtractor`, `PostedKeyset`, `DiscoverEmployerJobHandler`); циклов
пакетов нет (`normalize` ↔ `extract` устранён в §66); у изменённого поведения есть тесты; README,
план и `current-state.md` соответствуют коду; контракт цел (§0–§7, §0.6).

### 76.3 Нужно решение владельца

- **DTO-записи с большим числом полей.** `PostingDtos.Summary` (19) и `Card` — записи-DTO, у
  которых конструктор по определению имеет все поля; правило «≤5 параметров» (контракт §3.10)
  к ним формально применимо. Так устроены все DTO проекта с начала. Варианты: (а) явно
  исключить DTO-записи из правила в контракте; (б) группировать поля во вложенные записи
  (зарплата, локация), что меняет формат JSON. Это же определит настройку Checkstyle (регламент §6).
- **Значение по умолчанию потолка тела в трёх местах** (`application.yml`, `@DefaultValue` в
  `SourceHttpProperties`, константа для тестового конструктора `SourceHttpClient`) — оставлено:
  аннотация не может сослаться на константу-число; расхождение ловит `SourceHttpClientFetchTest`.

### 76.4 Автоматические проверки
...
## §79 — A2: принадлежность доски работодателю (Workday)

**Задача.** До сих пор `HIGH` означал «лента валидна, непуста, есть рынок SK/AT» — чья это
доска, не проверялось (A2/A11).

**Разведка (2026-09-24).** Имени работодателя Workday не отдаёт: в детали вакансии
`hiringOrganization` есть, но пустой (`{"name":"","url":""}`, IQVIA, `target/wd.json`); на
странице доски `<title>` и `og:title` пустые. Есть `tenant` (поддомен клиента),
`siteId` и `og:description` — описание работодателя свободным текстом («IQVIA is a leading
global provider…»).

**Решение владельца.** Тенант Workday выдаётся самому клиенту — это и есть принадлежность.
Риски — описание без имени владельца и кадровые агентства (публикуют вакансии чужих
работодателей). Их проверяет гейт; сомнение — не отказ, а ручная проверка (`PENDING`).
Имя компании — A3.

**Код.**
- `adapters/BoardProfile` — запись «владелец у провайдера + описание доски».
- `SourceAdapter.boardProfile(Source)` — по умолчанию `Optional.empty()` (Greenhouse: проверка
  не выполняется); `WorkdayAdapter` — GET `<base_url>/<site>`, `og:description` (Jsoup),
  владелец — тенант из `external_ref`.
- `discovery/BoardOwnershipProperties` (`app.discovery.ownership.agency-markers`) — `doubt()`:
  нет описания → «владелец доски не подтверждён»; признак агентства (подстрока без учёта
  регистра) → «похоже на кадровое агентство»; тенант не встречается в описании (сравнение
  только букв и цифр: `tobiidynavox` ↔ «Tobii Dynavox») → «описание доски не называет владельца».
  Паттерн как у `DiscoveryMarketProperties`: запись настроек с методом проверки.
- `DiscoverEmployerJobHandler`: `assess` → `assessFeed` (прежняя оценка ленты) +
  `withOwnership` — только для `CONFIRMED`, то есть один запрос на подключаемую доску.
  Сомнение → `LOW`/`PENDING`, причина дописывается перед прежней. Страница доски не
  открылась — тоже сомнение (не `UNREACHABLE`: лента читается); временный сбой — повтор
  задания, как у ленты (§57).
- Настройка зарегистрирована в `JobWorkerApplication`, значения — в `application.yml`.

**Тесты.** `BoardOwnershipPropertiesTest` — подтверждение (IQVIA, Tobii Dynavox), нет описания,
агентство, аббревиатура тенанта (`jnj`). `WorkdayDiscoveryStubTest` — стаб страницы доски
`/careers`: описание с тенантом → авто-подключение; агентство и нет описания → `PENDING`.
`DiscoverEmployerJobHandlerTest` — новый параметр конструктора (заглушка адаптера возвращает
пустой `Optional` — прежние сценарии не меняются).

**Попутно (после §78, до коммита):** в `PostingQueryServiceTest` и `InterviewApiIntegrationTest`
пути приведены к сгруппированному JSON (`facts().experience().yearsMin()`,
`$.application.status`).

**Не вошло.** Имя юрлица и дедуп работодателей (A3). Уже подключённые источники повторно не
проверяются. README — статус A2 (§0.2).

## §80 — A3: имя компании и одна компания на тенант

**Задача.** Компания заводилась с именем = slug («lilly/LLY»), и на каждый сайт — новая
компания: два сайта одного тенанта Workday давали двух «работодателей».

**Решение владельца.** Имя — из описания доски (Workday имени не отдаёт, §79); ключ дедупа —
провайдер + тенант. Дедуп между разными провайдерами (Greenhouse-seed ↔ Workday) — позже.

**Код.**
- `V28__company_identity_key.sql` — `company.identity_key TEXT`, `UNIQUE` (PostgreSQL допускает
  много `NULL` — у компаний не из обнаружения ключа нет). Заполнение для уже заведённых:
  старшая компания каждого тенанта получает ключ, прочие остаются без ключа (их связи не
  трогаются).
- `Company.identityKey(providerCode, slug)` — «код:slug до первого '/' в нижнем регистре»
  (`workday:lilly`; Greenhouse — сам slug). Статический метод в core: одинаково считается в
  job-api и job-worker.
- `CompanyRepository.findByIdentityKey` (job-api и job-worker).
- `EmployerSourceRegistrar` (авто) и `EmployerRegistration` (ручное подтверждение):
  компания ищется по ключу, иначе создаётся с ключом; имя известной компании не меняется.
- `BoardOwnershipProperties.ownerName` — то написание тенанта, которое нашла проверка A2:
  фрагмент описания с начала слова, чьи буквы и цифры совпадают с тенантом
  («tobiidynavox» → «Tobii Dynavox», «iqvia» → «IQVIA»).
- `DiscoverEmployerJobHandler`: `Assessment` получил `companyName` (5-е поле; прежний
  4-аргументный конструктор оставлен для остальных оценок, `named()` — с именем). Имя не
  найдено (Greenhouse, нет профиля) — slug, как раньше.

**Тесты.** `EmployerSourceRegistrarTest` — ключ у новой компании; второй сайт тенанта берёт
прежнюю компанию; формат ключа. `BoardOwnershipPropertiesTest` — `ownerName`.
`WorkdayDiscoveryStubTest` — компания получает имя «Acme» из описания доски.

**Не проверено в среде ассистента:** миграция на PostgreSQL (проверит Flyway в интеграционных
тестах), компиляция классов со Spring/JPA.

**Не вошло.** Дедуп между провайдерами; переименование компаний, заведённых до V28 (имя —
slug). Гонка двух заданий одного тенанта в одну секунду: второе упадёт на
`uq_company_identity_key` (дальше — штатная обработка сбоя задания); дубль компании не
возникнет. README — статус (§0.2).

## §81 — Разбор адреса Workday: все ведущие локали; скелет A5 (покрытие)

Два пункта одним срезом по решению владельца.

### 81.1 Разбор адреса доски Workday

**Дефект (стенд §80).** Шесть досок получили код языка вместо сайта: `salesforce/en-us`,
`capitalone/en-Uk`, `crowe/en-us` … → лента 404 → `UNREACHABLE`. В индексе Common Crawl есть
адреса с двумя локалями подряд (`/en-US/en-us/...`); `WorkdayBoard.siteFromPath` пропускал
только первую.

**Исправление.** Ведущие локали пропускаются все подряд; после них нет сайта — доски нет.
Уже записанные кандидаты с локалью вместо сайта остаются `UNREACHABLE`; правильная доска —
другой slug, она появится при обходе следующей коллекции индекса (пройденные страницы текущей
коллекции повторно не читаются). Ограничение: сайт из двух
строчных букв теперь неотличим от локали (реальных примеров не встречалось).
Тест — `WorkdayBoardTest.consecutiveLocalesAreSkipped`.

### 81.2 A5 — скелет оценки покрытия

**Решение владельца.** Эталонная площадка для сравнения ещё не выбрана (протокол Этапа 0), поэтому
в этом срезе — модель и чтение; сравнение — после выбора площадки.

**Модель (ADR-15).** Покрытие — результат сравнения, а не флаг.
- `CoverageState`: `UNKNOWN` (не проверено/неполно), `SITE_ONLY` («скрытая»), `ON_PLATFORM`,
  `BOTH`.
- `CoverageAssessment` (core): одна запись на публикацию — состояние, проверенные площадки,
  причина, дата проверки, версия сопоставления. **Нет записи = `UNKNOWN`**: заполнять таблицу
  для существующих публикаций не нужно.
- `V29__coverage_assessment.sql`: `UNIQUE (job_posting_id)`, `CHECK` на состояние, индекс по
  состоянию (фильтр), `ON DELETE CASCADE` — оценка часть публикации (тесты и очистка удаляют
  публикации).

**API (изменение формата JSON — согласовано владельцем).**
- Строка ленты и карточка получили `coverage` = `{state, checkedPlatforms, reason, checkedAt}`;
  без оценки — `{state: "UNKNOWN", reason: "сравнение с площадками не выполнялось"}`
  (`Coverage.NOT_CHECKED`).
- Фильтр `GET /api/v1/postings?coverage=SITE_ONLY` — «только скрытые»; `UNKNOWN` — ещё и
  публикации без оценки. Отдельный query-параметр, а не поле `PostingFilter`: у записи уже 5
  полей (контракт §3.10), прежние имена параметров не меняются.
- JPQL (`PostingReadRepository.FILTERS`): `exists` по оценке с нужным состоянием; признак
  «запрошено UNKNOWN» вычисляется в SpEL (`#coverage.name() == 'UNKNOWN'`), как у даты (§68).
- `PostingQueryService`: оценки страницы — одним запросом `findByPosting_IdIn` (без N+1);
  карточка — `findByPosting_Id`. Новый параметр `coverage` в `list` и в методах репозитория.

**Тесты.** `PostingQueryServiceTest` — сигнатуры и новый сценарий (оценка → `SITE_ONLY`, без
оценки → `UNKNOWN`). `PostingApiIntegrationTest.coverageUnknownByDefaultAndFilterBySiteOnly` —
фильтр `SITE_ONLY`/`UNKNOWN` и поле в карточке на PostgreSQL.

**Не вошло.** Эталонная площадка, её адаптер, сопоставление и запись оценок; пересчёт при смене
набора площадок. README — статус (§0.2).

## §82 — A5: сравнение с karriere.at (Австрия)

**Выбор площадки (владелец).** Австрия — karriere.at: robots.txt разрешает обход, AGB без явного
запрета автоматического чтения; хранится только факт наличия (правовое основание — протокол
Этапа 0, п.5). profesia.sk (Словакия) отложена: robots.txt запрещает поиск с параметрами и
страницы компаний.

**Разведка (`target/recon-karriere.sh`, 5 работодателей).** Выдача `karriere.at/jobs/<слово>` —
HTML без встроенного JSON; активные вакансии — блок `.m-jobsSearchList__activeJobs` (у каждой
`/jobs/<id>`, заголовок, юрлицо), архивные — отдельно. Поиск полнотекстовый: по «hitachi» есть
LINDLPOWER и CANCOM. Итог по нашим австрийским вакансиям: Accenture 15 → 0 на площадке, IQVIA
4 → 0, Hitachi 12 → 1 точно и 1 почти, Ecolab — тот же заголовок у «Ovivo Aqua Austria GmbH».

**Правила (владелец: строгие + «не проверено», ADR-15).** `CoverageMatcher`:
- заголовок совпал после нормализации (регистр, пометки пола `(m/w/d)`/`(f/m/d)`/`(all genders)`,
  знаки) у юрлица, содержащего имя работодателя → `BOTH`;
- тот же заголовок у другого юрлица → `UNKNOWN` (дочерняя компания или агентство);
- у работодателя «почти такой же» заголовок (общих слов ≥ 75%, коэффициент Жаккара) →
  `UNKNOWN`;
- выдача неполная (в заголовке выдачи вакансий больше, чем разобрано) → `UNKNOWN`;
- иначе `SITE_ONLY` «не найдена на karriere.at при проверке [дата]».
Версия правил `karriere-1` пишется в оценку.

**Код (job-worker, пакет `coverage`).**
- `KarriereClient` — запрос через `SourceHttpClient` (темп `[karriere.at]: 10000` мс), разбор
  Jsoup; нет блока активных вакансий — ошибка (смена вёрстки не превращается в «всё скрытое»).
- `CoverageCheck` — под leader-lock (ключ 1005) до `max-companies-per-pass` работодателей с
  австрийскими публикациями без свежей оценки (`recheck-after-hours`), один запрос на
  работодателя, оценка на каждую публикацию (создать или обновить). Сбой площадки — оценки не
  трогаются, работодатель берётся снова. Имя для поиска — имя компании; у заведённых до §80
  (slug) — часть до `/`.
- `CoverageTargetRepository` (кого и что проверять), `CoverageAssessmentRepository`,
  `CoverageProperties` (`app.coverage`), `CoverageTrigger` (тик; в тестах выключен).

**Тесты.** `CoverageMatcherTest` — случаи разведки (точное, почти, чужое юрлицо, не найдено,
неполная выдача, нормализация). `KarriereClientTest` — разбор по вёрстке (только активные,
пропуск рекламного блока, пустая выдача, неполная, чужая страница — ошибка, слово поиска).
`CoverageCheckTest` — запись оценок, сбой площадки без записи.

**Не проверено в среде ассистента:** разбор Jsoup (Jsoup недоступен — селекторы проверены на
сохранённых страницах через Python/BeautifulSoup), запросы JPQL, Spring-контекст.

**Не вошло.** Словакия; пагинация выдачи (неполная выдача → «не проверено»); длинная
транзакция лока на время запросов (бюджет — 3 запроса за проход). README — статус (§0.2);
протокол Этапа 0 — площадка сравнения.

## §83 — Покрытие: «нет выдачи» у площадки; имена компаний, заведённых до §80

Два пункта одним срезом по решению владельца (стенд §82).

### 83.1 404 от karriere.at

**Дефект.** По слову «dxctechnology» площадка ответила 404 (так она отвечает, когда выдачи по
слову нет). `CoverageCheck` считал это сбоем и ничего не записывал: у компании самый малый `id`,
и она занимала одно из трёх мест в каждом проходе.

**Исправление.** 404 → всем австрийским публикациям работодателя `UNKNOWN` «на karriere.at нет
выдачи по «X» при проверке [дата]» (`checkedAt` — работодатель свежий до перепроверки через
`recheck-after-hours`). Прочие сбои — как раньше, без записи. Метод прохода разделён:
`checkCompany` на одного работодателя.

### 83.2 Имена старых компаний

**Задача.** Компании, заведённые до §80, названы slug (`lilly/LLY`, `dxctechnology/DXCJobs`):
поиск на площадке идёт по тенанту и не находит бренд.

**Решение.** `CompanyNameBackfill` (job-worker, `discovery`): раз в час под leader-lock (ключ
1006) до 10 компаний с `/` в имени (`CompanySourceRepository.withSlugNames` — связь, компания,
источник и провайдер одним запросом) получают имя как новые (A3): сведения о доске
(`SourceAdapter.boardProfile`) → `BoardOwnershipProperties.ownerName`. Нет имени в описании,
провайдер сведений не даёт или страницы доски нет (4xx, кроме 429) — имя = владелец из slug:
без `/` компания больше не выбирается, проход не зацикливается. Временный сбой (429, темп, сеть,
5xx) — slug остаётся, повтор в следующем проходе. `identity_key` не меняется.
Настройки — `app.discovery.name-backfill` (включено; в тестах выключено).

**Тесты.** `CoverageCheckTest.noResultsPageMarksPostingsUnknownWithDate`;
`CompanyNameBackfillTest` — имя из описания («Tobii Dynavox», «DXC Technology»), владелец из
slug при 404, slug при 429.

**Не вошло.** Словакия; ручное переименование компании через API. README — без изменений
(внутреннее исправление, §0.2).

## §84 — Итог дня: стенд §83 и открытые вопросы (только документация)

**Состояние.** Срезы §77–§83 закоммичены, CI зелёный. Сделано за день: Checkstyle (≤5 параметров,
≤5 полей записи), A2 (принадлежность доски Workday), A3 (имя компании, одна компания на тенант,
V28), A5 для Австрии (оценка покрытия V29, сравнение с karriere.at, фильтр `coverage`),
исправления разбора адреса Workday и ответа 404 площадки, переименование старых компаний.

**Стенд §83 (`target/stand-coverage.sh`, два прохода).**
- Покрытие: Hitachi — 3 «и там, и там», 7 «только с сайта», 2 «не проверено» (похожий
  заголовок); Ecolab — 2 «только с сайта», 1 «не проверено» (тот же заголовок у Ovivo Aqua);
  IQVIA 4, Snapchat 3, Accenture 15 — «только с сайта»; DXC — «не проверено» (нет выдачи).
- Имена: 10 компаний за проход («Ace Hardware», «ACRT», «Amgen», «2020 Companies»); нет имени в
  описании — владелец из slug («amat», «aenetworks»). Осталось около 60 компаний со slug.

**Открыто (ждёт решения владельца).**
1. Дубли компаний, заведённых до V28: у тенанта несколько компаний (ABC Supply ×4, Ace Hardware
   с разными именами по сайтам); V28 дала ключ только старшей. Предложение: перенести связи с
   досками и подписки на компанию с ключом, лишние удалить.
2. Бренд сайта ≠ тенант: accenture/AvanadeCareers (14 вакансий Avanade искались как
   «accenture» → «только с сайта»), capri/Michael_Kors, pae/Amentum_Careers,
   signetjewelers/Banter. Возможная ложная «скрытая». Предложение: при явном расхождении —
   «не проверено» или поиск ещё и по имени сайта.
3. Площадка для Словакии (profesia.sk закрыта robots.txt); ArchUnit в CI.

Аудит по регламенту — через 2 среза (счётчик 8).

## §85 — Объединение дублей компаний, заведённых до V28

Пункт 1 открытых вопросов §84, по решению владельца.

**Проблема.** До ключа идентичности (V28, §80) каждый сайт тенанта Workday получал свою
компанию: у тенанта abcsupply — четыре компании, у ace — две с разными именами. V28 дала ключ
«провайдер:тенант» только старшей (меньший `id`); остальные остались дублями без ключа. Лента,
покрытие и подписки видели одного работодателя как несколько.

**Решение — миграция данных `V30__merge_duplicate_companies.sql` (job-api).** Одноразовая, в
одной транзакции (Flyway оборачивает каждую миграцию PostgreSQL в транзакцию):
- **Кто дубль.** Компания без ключа, у которой все связи с источниками ведут к одному ключу
  (формула та же, что в V28: `код провайдера:тенант до '/' в нижнем регистре`), и этот ключ уже
  есть у другой компании. Компании без связей (заведённые вручную), со связями с разными
  тенантами или с ключом, которого нет ни у кого, не трогаются.
- **Пары «дубль → компания с ключом»** — во временной таблице `company_merge`
  (`CREATE TEMPORARY TABLE … ON COMMIT DROP` — таблица сеанса, исчезает при завершении
  транзакции; https://www.postgresql.org/docs/16/sql-createtable.html).
- **Перенос ссылок** на компанию с ключом: `company_source`, `company_subscription`,
  `notification`, `employer_candidate` — это все таблицы с внешним ключом на `company`.
  Где есть уникальность по компании (`(source_id, company_id)`, `(app_user_id, company_id)`),
  сначала удаляется строка, которая после переноса повторила бы уже существующую: у компании с
  ключом такая строка есть, или её получит другой дубль с меньшим `id` строки. Пример: пользователь
  подписан на три компании abcsupply — остаётся одна подписка.
- **Удаление дублей** — после переноса ссылок на них не остаётся.
- Имя остаётся у компании с ключом; slug-имена исправляет `CompanyNameBackfill` (§83).
  `identity_key` не меняется. Повторного запуска у Flyway нет, но и повторное выполнение SQL
  ничего бы не изменило: дублей после первого нет.

```mermaid
flowchart LR
  D1[abcsupply/B<br/>без ключа] --> M{company_merge}
  D2[abcsupply/C<br/>без ключа] --> M
  M -->|связи, подписки,<br/>уведомления, кандидаты| K[ABC Supply<br/>workday:abcsupply]
  M -->|затем| X[дубли удалены]
```

**Тест.** `CompanyMergeMigrationIntegrationTest` (job-api, Testcontainers без Spring-контекста):
Flyway накатывает схему до V29 (`target("29")`), в базу кладутся данные «как до V28» — три дубля
abcsupply (два из них связаны с одной доской, один — с доской компании с ключом), дубль ace,
компания с двумя тенантами, компания без связей, подписки двух пользователей, уведомления,
кандидаты; затем применяется V30. Проверяется: удалены только дубли; связи с досками и подписки
перенесены без повторов; уведомления и кандидаты указывают на компанию с ключом. Тот же набор
данных ассистент прогнал на PostgreSQL 16 до выдачи — результат совпал с ожиданиями теста;
компиляция теста не проверялась (Maven Central недоступен в среде ассистента).

**Стенд (локальная база владельца).** До V30 у 9 тенантов было больше одной компании:
acehardware 5, abcsupply 4, signetjewelers 3, acrt 3, ghr, medtronic, mmc, pfizer, accenture —
по 2. После V30 — ни одного. Сборка `mvn verify` зелёная (job-api 146 тестов, job-worker 207).
Следствие для §84 п. 2: доски accenture/AvanadeCareers теперь у одной компании «accenture» —
так и задумано ключом «провайдер:тенант»; вопрос бренда сайта остаётся открытым.

**Не вошло.** Объединение компаний разных тенантов одного бренда и между провайдерами;
расхождение бренда сайта и тенанта (§84, п. 2). README — без изменений (внутренняя правка
данных, §0.2).

## §86 — Покрытие по бренду доски; ArchUnit в сборке

Два пункта открытых вопросов §84 одним срезом по решению владельца (срезы крупнее).

### 86.1 Бренд сайта ≠ тенант

**Проблема.** Проверка покрытия искала на karriere.at один раз на компанию — по её имени. У тенанта
бывают сайты других брендов (`accenture/AvanadeCareers`, `capri/Michael_Kors`,
`pae/Amentum_Careers`, `signetjewelers/Banter`); их вакансии на площадке опубликованы под брендом,
поиск «Accenture» их не находил — ложное «только с сайта» (стенд §83: 14 вакансий Avanade). После
V30 (§85) доски такого тенанта — у одной компании, поэтому различать нужно по доске.

**Решение.**
- `BoardBrand` (job-worker, `coverage`): бренд из имени сайта (часть адреса доски после `/`).
  Имя делится на слова по `_`, `-`, цифрам и смене регистра (`AvanadeCareers` → «Avanade Careers»),
  общие слова сайтов (`SITE_WORDS`: Careers, Jobs, External, Search, Site…) отбрасываются. Бренда
  нет, если слов не осталось, если он короче 3 букв (`MIN_BRAND_LENGTH`) или совпадает с именем
  компании или тенантом (одно содержит другое): `lilly/LLY`, `dxctechnology/DXCJobs`, `mmc/MMC` —
  поиск по имени компании, как раньше. Список слов — константа, а не настройка: это свойство
  имён сайтов Workday, а не окружения.
- `CoverageCheck`: публикации работодателя группируются по слову поиска
  (`searchName(company, source)` — бренд доски или имя компании); на каждую группу — один запрос к
  площадке (`checkEmployer`, прежняя логика: 404 → «не проверено», сбой → без записи). Бюджет прохода
  по-прежнему в работодателях: работодатель с доской другого бренда даёт на запрос больше.
- `CoverageTargetRepository.postingsOf` — `join fetch p.source`: имя сайта читается без отдельного
  запроса на каждую публикацию (нет N+1).

### 86.2 ArchUnit в сборке (план C4)

**Что это.** ArchUnit (https://www.archunit.org) — библиотека, которая читает скомпилированные
классы и проверяет правила зависимостей между ними обычным тестом JUnit. Подключено **ядро**
`com.tngtech.archunit:archunit` 1.5.0 (версия — в корневом `pom.xml`, свойство `archunit.version`;
зависимость — только `test` в job-api и job-worker). Собственный движок JUnit библиотеки
(`archunit-junit5`) не используется: правила вызываются из обычных `@Test`, так не нужна
совместимость движка с JUnit, который приносит Spring Boot 4.

**Правила** (`ArchitectureTest` в каждом модуле, классы без тестов):
- job-worker: нет циклов между пакетами верхнего уровня; `java.net.http`, `HttpURLConnection`,
  `RestClient`, `RestTemplate` и `Jsoup.connect` — только в пакете `http` (`SourceHttpClient`);
  парсеры XML (`javax.xml.parsers/stream/transform/validation`, `org.xml.sax`) — только в пакете `xml`
  (`SafeXml`).
- job-api: нет циклов между пакетами верхнего уровня; классы `@RestController` не зависят от
  репозиториев Spring Data.

**Найденные нарушения — исправлены.**
- Цикл `posting ↔ saved`: лента читает маркеры, а `SavedPostingService` брал публикацию из
  `posting.PostingReadRepository`. Теперь — `saved.MarkedPostingRepository` (узкий
  `Repository<JobPosting, Long>` с одним `findById`); тест сервиса обновлён.
- `AuthController.me` читал `AppUserRepository` напрямую. Теперь — `AppUserService.current(email)`
  (тот же `401 «Сессия недействительна»`, маршрут и ответ не изменились).

**Тесты.** `BoardBrandTest` — бренды Avanade, Michael Kors, Amentum, Banter; без бренда — общие и
совпадающие имена сайтов. `CoverageCheckTest.boardOfAnotherBrandIsSearchedByBrand` — два запроса:
«Accenture» (своя доска, «только с сайта») и «Avanade» («и там, и там»). `ArchitectureTest` ×2.
Проверено ассистентом: `BoardBrand` скомпилирован и прогнан на 14 именах досок (результаты — как в
тесте). Остальное не компилировалось (Maven Central недоступен в среде ассистента).

**Стенд.** `target/stand-brand.sh` (вне git): состаривает оценки публикаций тенантов accenture, capri,
pae, signetjewelers, запускает воркер на 3 минуты (до 20 работодателей за проход), отчёт
`target/stand-brand.txt` — оценки по доскам и строки лога «Покрытие» со словом поиска.

**Документы.** README — строка CI (§0.2: изменился состав проверок сборки); регламент §6 — ArchUnit
введён; план — C4 отмечен; `application.yml` — комментарий бюджета прохода.

**Не вошло.** Бренд из описания страницы доски (`og:description`) — нужен запрос на доску;
объединение брендов между тенантами и провайдерами; Словакия.

## §87 — Аудит (срезы §77–§86)

Срез-аудит по регламенту сессии §5 (счётчик достиг 10). Область — код, изменённый с прошлого
аудита (`git diff 9011435..e15c08d`: 53 файла кода, 40 тестов, миграции V28–V30, конфигурация) и
документы. Новой функциональности нет, поведение не меняется.

### 87.1 Найдено и исправлено

1. **Однобуквенные имена (контракт §3.1)** в изменённых файлах: переменные `catch (… e)` → `exception`
   (27 блоков, в том числе `isUnreachable`/`isTransient` в `DiscoverEmployerJobHandler`), параметры
   лямбд (`l`, `s`, `b`, `k`, `e`, `r`, `i`) → по смыслу (`listing`, `segment`, `board`, `absent`,
   `entry`, `summary`, `response`, `invocation`, `event`), индекс цикла `i` → `lineIndex`
   (`CommonCrawlIndexClient`), в тестах — `e` → `error`/`backoff`, `m` → `market`.
2. **Числа без имени (§3.1):** `DiscoverEmployerJobHandler.SUMMARY_ITEMS = 3` (сколько стран/локаций
   в обосновании гейта), `CommonCrawlIndexClient.LOG_TAIL_CHARS = 80`, `RequestPacer.DOMAIN_LABELS = 2`,
   `GreenhouseAdapter.CENTS_SCALE = 2`.
3. **Дубль:** `DiscoverEmployerJobHandler.assessMarket` повторял код `topOf` (три главные страны) —
   теперь вызывает `topOf`.
4. **Реестр ключей leader-lock** был разбросан по комментариям трёх классов и в каждом неполон
   (риск взять занятый ключ). Реестр 1001–1006 — в JavaDoc `PostgresLeaderLock`; у констант — ссылка
   на него.

### 87.2 Проверено, нарушений нет

Параметры и поля записей ≤5 (Checkstyle); JavaDoc у классов и публичных методов (у `@Override` —
из интерфейса); SQL только в репозиториях; `@Transactional` — у сервисов записи и чтения; N+1 в новых
запросах нет (`withSlugNames`, `postingsOf` — `join fetch`); внешние вызовы — через
`SourceHttpClient` с тайм-аутами (ArchUnit); тесты есть у всего изменённого поведения, тестов удалённых
классов (`CertSpotterClient`, `WorkdaySiteResolver`) нет; README, `current-state.md`, план, регламент
§6 и комментарии `application.yml` соответствуют коду; контракт цел (§0–§7).

**Пустая выдача karriere.at (вопрос из §86) — поведение верное.** Разведка §82: страница без
вакансий приходит с заголовком без числа («Accenture Jobs») и пустым блоком активных — это
«активных вакансий нет», а не сбой вёрстки (тест `KarriereClientTest.emptyResultIsComplete…`); сбой
вёрстки уже даёт ошибку (нет блока) или неполную выдачу (объявлено больше, чем разобрано). Значит,
15 вакансий Accenture/Avanade «только с сайта» — ожидаемый сигнал гипотезы, а не ложный.

### 87.3 Нужно решение владельца

- **`catch (… e)` в остальном коде.** Исправлено только в изменённых с §76 файлах; в прочих
  (≈40 файлов до §66) `e` осталось. Варианты: (а) переименовать везде и добавить в Checkstyle
  `LocalVariableName`/`LambdaParameterName`/`CatchParameterName` с минимальной длиной 2 — правило
  станет проверяться сборкой; (б) оставить как есть.
- **`CompanyNameBackfill`** — разовая задача (осталось около 60 slug-имён, по 10 в час). После
  того как slug-имён не останется, класс, его настройки и тест можно удалить; тогда уйдёт и
  отличие от остальных фоновых задач (тик внутри компонента, размер пачки — константа, а не настройка).
- `docs/stage-0-protocol.md` упоминает удалённые `CertSpotterClient`/`WorkdaySiteResolver` — это
  протокол Этапа 0 (история), оставлен без изменений.

### 87.4 Автоматические проверки

Checkstyle (§78) и ArchUnit (§86) работают в `mvn verify` и CI. Счётчик срезов в
`current-state.md` сброшен в 0. README — без изменений (внутренняя правка, §0.2).

## §88 — Имена в Checkstyle; исследование площадки Словакии; A7 — дайджест по email

Три пункта одним срезом по решению владельца (§87.3, A5 для Словакии, A7). A6 (снимки в S3) —
следующим срезом, хранилище выбрано: MinIO локально + AWS SDK v2.

### 88.1 Однобуквенные имена — во всём коде и в сборке

Контракт §3.1 («нет имён из одного символа») теперь проверяет Checkstyle
(`config/checkstyle/checkstyle.xml`): `LocalVariableName`, `LocalFinalVariableName`, `ParameterName`,
`LambdaParameterName`, `CatchParameterName`, `PatternVariableName`, `RecordComponentName`,
`MemberName` с форматом `^[a-z][a-zA-Z0-9]+$` — строчная буква и ещё хотя бы один символ; индексы
циклов не исключены (https://checkstyle.org/checks/naming/). Исправлено 57 нарушений в коде до
§77: `catch (… e)` → `exception` (15), лямбды и локальные переменные по смыслу (`marker`, `term`,
`country`, `entry`, `octets`, `index`, `letters`, `part`, `word`, `rowNum`, `httpHeaders`), в тестах —
`error`, `backoff`, `sessionA`/`sessionB`. Проверено ассистентом: Checkstyle 10.21.4 с новой
конфигурацией по `core`, `job-api`, `job-worker` — 0 нарушений.

`CompanyNameBackfill` не удалён: удаляется, когда в базе не останется компаний с `/` в имени
(запрос — в ответе среза); удаление — вместе с §89.

### 88.2 Площадка для Словакии — исследование (кода нет)

Проверено 2026-09-25 (robots.txt и страницы выдачи):

| Площадка | robots.txt | Поиск по работодателю | Вывод |
|---|---|---|---|
| **profesia.sk** (крупнейшая в SK) | запрещены `/*search_offers*form=`, `/*company_details.php`, `/sk/`, `/cz/`, `/de/`, `/en/*?` и служебные страницы; `/praca/?search_anywhere=…` **ни под одно правило не попадает** | `/praca/?search_anywhere=accenture` — 17 вакансий, у каждой юрлицо («Accenture, s.r.o.») и ссылка `/praca/<фирма>/O<id>`, число в заголовке | подходит так же, как karriere.at |
| kariera.sk (Zoznam) | запрещены поиск `/*?q=*` и `/sk/`; страницы фирм `/ponuky-spolocnosti/<slug>` разрешены | только по slug фирмы из sitemap (~1 100 фирм, в основном местные; Accenture, Hitachi, DXC, IBM нет); страницы разделов отвечают 403 | не подходит |
| worki.sk (бывший istp.sk, TREXIMA) | почти всё разрешено | ~1 900 вакансий всего | слишком мала для сравнения |

Прежний вывод §82 («robots.txt profesia.sk запрещает поиск с параметрами и страницы компаний»)
для поиска `/praca/?search_anywhere=` сейчас неверен: запрещён поиск через форму (`search_offers…form=`)
и карточка фирмы `company_details.php`. **Рекомендация:** profesia.sk, поиск
`/praca/?search_anywhere=<слово>`, та же модель, что у karriere.at (только факт наличия, темп на
домен, `SourceHttpClient`). **Нужно решение владельца:** проверить условия использования
profesia.sk (автоматическое чтение) — как для karriere.at (правовое основание A24). Реализация —
в §89 (клиент площадки SK, `CoverageCheck` по стране).

### 88.3 A7 — ежедневный дайджест по email

**Что делает.** Раз в день (cron `app.digest.mail.cron`, по умолчанию 06:50 Europe/Bratislava) каждому
пользователю с подписками на компании уходит письмо за окно «от прошлого письма до сейчас»:
- **новые вакансии** отслеживаемых компаний (`job_posting.first_seen_at` в окне), сначала
  «только на сайте компании» (оценка покрытия `SITE_ONLY`) — ради них продукт;
- **изменения** известных вакансий — из уведомлений (`notification`, их создаёт сопоставление
  подписок §39); одна строка на вакансию, поля словами без повторов («зарплата, место»);
  вакансии, появившиеся в том же окне, в изменения не попадают (их первое заполнение — не изменение).
Нет ни того, ни другого — письма нет. В каждом разделе до `max-items` строк, дальше «и ещё N».

**Окно и baseline (A16).** Начало окна — конец последнего заведённого окна пользователя; если писем
ещё не было — момент первой подписки: история до подписки не отправляется.

**Доставка с состоянием** (`digest_delivery`, V31): строка на (пользователь, окно), `UNIQUE
(app_user_id, window_end)`; `PENDING` → `SENT`, при ошибке SMTP — повтор в следующем проходе,
после `max-attempts` — `FAILED`. Текст письма не хранится — собирается из данных окна при каждой
попытке. SMTP не «ровно один раз»: падение после приёма письма сервером, но до фиксации транзакции
даёт повтор (допустимо по technical-design A16).

**Конструкции.**
- `spring-boot-starter-mail` — стандартный модуль Spring Boot для почты: `JavaMailSender`
  настраивается из `spring.mail.*` (хост, порт, логин, тайм-ауты)
  (https://docs.spring.io/spring-boot/reference/io/email.html). Письмо — `SimpleMailMessage`,
  простой текст.
- Тайм-ауты SMTP (`mail.smtp.connectiontimeout/timeout/writetimeout`) в `application.yml` —
  зависший сервер не держит проход (контракт §3.4).
- **Mailpit** (`axllent/mailpit`, docker-compose, `dev-up.sh`) — локальная «ловушка» писем: принимает
  SMTP на 1025, показывает письма на http://localhost:8025, наружу ничего не отправляет
  (https://mailpit.axllent.org). В облаке `MAIL_HOST`/`MAIL_PORT`/`MAIL_USERNAME`/`MAIL_PASSWORD` —
  адрес SMTP-провайдера.
- `@Scheduled(cron = …, zone = …)` — запуск по календарю в заданном часовом поясе
  (https://docs.spring.io/spring-framework/reference/integration/scheduling.html).

**Код (job-worker, пакет `digest`).** `DigestRun` — проход под leader-lock 1007: заводит письма
(`plan`), отправляет `PENDING` (до 200 за проход); `DigestTrigger` — ежедневный запуск
(`app.digest.mail.enabled`, в тестах выключен); `DigestLookup` — SQL получателей и содержимого окна
(как `SubscriptionLookup`: выборка через подписки, связи с досками, публикации, оценки покрытия);
`DigestComposer` — тема и текст; `DigestContent`, `DigestDelivery` (+ состояние, репозиторий),
`DigestProperties` (`app.digest.mail`: `enabled`, `from`, `zone`, `max-attempts`, `max-items`).

```mermaid
flowchart LR
  T[DigestTrigger<br/>cron 06:50] --> R[DigestRun<br/>lock 1007]
  R -->|plan: окно с прошлого письма| L[(DigestLookup:<br/>новые вакансии + уведомления)]
  R -->|PENDING| D[(digest_delivery)]
  D --> C[DigestComposer] --> M[JavaMailSender<br/>SMTP / Mailpit]
  M -->|принято| S[SENT]
  M -->|ошибка| P[attempts+1 → PENDING / FAILED]
```

**Тесты.** `DigestComposerTest` — разделы, пометка покрытия, поля словами, «и ещё N», пустой раздел
не выводится. `DigestRunTest` — письмо за окно от подписки, пустое окно — без письма, сбой SMTP —
`PENDING` до третьей попытки, затем `FAILED`. SQL `DigestLookup` проверен ассистентом на PostgreSQL 16
со схемой V1–V31 (получатель и начало окна, новые вакансии с `SITE_ONLY` первыми, изменения без
новых вакансий, `CHECK` окна); компиляция не проверялась (Maven Central недоступен в среде ассистента).

**Стенд.** `target/stand-digest.sh` (вне git): пользователь `stand-digest@example.test` с подписками на
3 компании «задним числом», воркер с дайджестом раз в минуту, письма читаются из Mailpit →
`target/stand-digest.txt`.

**Не вошло.** Частота по выбору пользователя (решено: раз в день для всех); «вероятно закрытые»
вакансии (бизнес-ТЗ §7.4) — признака закрытия в данных ещё нет; HTML-письмо; отписка по ссылке из
письма; дайджест в приложении (есть уведомления §39). README — обновлён (почта в стеке, Mailpit в
локальном запуске, дайджест в статусе, §0.2).

## §89 — Документы: план работ сжат, протокол Этапа 0 закрыт, реестр правовых оснований (только документация)

По вопросу владельца: нужны ли ещё `roleorienta-plan-korrektirovka.md` и `stage-0-protocol.md`.

- **План работ** (`roleorienta-plan-korrektirovka.md`) — нужен: это текущий список задач, его
  сверяет аудит (регламент §5). Сжат до списка задач: убраны проверка внешнего разбора от
  2026-09-22 (разделы 0–3) и применённые правки документации (раздел 5) — они в истории git.
  Отметки приведены к коду: сделаны A1 (§72), B1 (§57, §63), D4 (§87) — раньше не отмечены;
  частично — A2, A3, A4, A5 с пояснением; открытые вопросы — правовое основание Workday,
  условия profesia.sk, SPA. Вопрос «ниша и рынок» снят — паспорт пилота в протоколе Этапа 0.
- **Протокол Этапа 0** (`stage-0-protocol.md`) — закрыт и сохранён как записанный протокол
  (технический документ §14, ADR-17): пункт «первый целевой тенант» снят (тенантов подключает
  автоматический вход), правовые основания перенесены в реестр; добавлен итог (§7).
- **Реестр правовых оснований (A24)** — новый раздел `current-state.md`: источник, как читаем,
  основание с датой проверки, что храним. **Найдено:** для лент Workday — основного источника,
  откуда хранится и текст вакансии, — основание нигде не записано. Нужно решение владельца.
- `current-state.md` — «Статус плана» сокращён со ссылкой на план; следующий срез — §90 (A6).

Код не менялся. README — без изменений (§0.2).

## §90 — Правовые основания источников: проверка условий Workday, karriere.at, profesia.sk (только документация)

По поручению владельца ассистент проверил условия сам (2026-09-25). Это чтение robots.txt и
условий использования, не юридическое заключение.

**Workday (ленты тенантов, основной источник).**
- robots.txt хостов тенантов (`lilly.wd115`, `accenture.wd103`): `Allow` для путей сайтов
  вакансий, `Disallow: /refreshFacet/`; `/wday/cxs/` не закрыт — по стандарту robots.txt разрешён.
- Условия Workday (https://www.workday.com/en-us/legal/site-terms.html) действуют для workday.com,
  страниц, ссылающихся на них, Community и «Workday APIs», которые Workday делает доступными «on or
  through the Sites» (нужен отдельный договор); там же запрет scraping «our Sites». Сайты вакансий
  клиентов на myworkdayjobs.com не названы; на страницах вакансий и в ответе `cxs` ссылок на условия нет.
- Итог: запрета не найдено, **серая зона** — `cxs` можно трактовать как «Workday API». Отдельный
  риск — наш API отдаёт полный текст вакансии в карточке (перепубликация текста работодателя).
  Не сделано и стоит сделать: узнаваемый User-Agent с контактом, программное соблюдение robots.txt.

**karriere.at.** Nutzungsbedingungen (https://www.karriere.at/nutzungsbedingungen), п. 2.8.3:
«Sämtliche Formen der automatisierten Auswertung unserer Plattform sind verboten»; п. 1.3 — без
согласия с условиями пользоваться нельзя (все посетители). Вывод §82 («AGB без явного запрета»)
неполон: проверялись AGB, а запрет — в Nutzungsbedingungen. **Проверка покрытия karriere.at
(`app.coverage.enabled: true`) условиям противоречит** — нужно решение владельца.

**profesia.sk.** Условия Alma Career для пользователей, Словакия (действуют с 01.01.2025,
https://www.almacareer.legal/user/slovakia/documents/terms-of-use-of-alma-career-electronic-systems-for-users-eng):
«пользователь» — любой, кто пользуется системой, в том числе незарегистрированный; запрещены
попытки «automatically or collectively read or copy the content» (п. 4.7 e), автоматическая
обработка и data mining (п. 4.11 a, d), копирование и иная техническая обработка без согласия
(п. 5.6). Выгрузка содержимого (XML) — только по письменному договору (условия онлайн-партнёрства
Profesia). Вывод §88.2 «подходит так же, как karriere.at» отменяется: robots.txt разрешает, условия — нет.

**Изменено.** Реестр в `current-state.md` (колонка «Итог»), открытые вопросы и A5 в плане. Код не
менялся. README — без изменений.

## §91 — Проверка karriere.at выключена; системы найма малого бизнеса; адаптер и вход Personio

Три пункта по решению владельца после §90. Владелец: фокус — карьерные страницы самих компаний;
метка «скрытая» мало что добавляет.

### 91.1 Проверка покрытия выключена

`app.coverage.enabled: false` в `application.yml` (с комментарием): условия karriere.at запрещают
любое автоматическое чтение (§90). Уже записанные оценки остаются, новые не делаются. Код покрытия,
поле `coverage` в API и фильтр не тронуты — их судьба (метка «скрытая») ждёт решения владельца.

### 91.2 Исследование: системы найма малого и среднего бизнеса (2026-09-25)

Цель — источники с явным разрешением на чтение, а не серая зона, и работодатели не только из крупных.

| Система | Лента | Условия чтения | Итог |
|---|---|---|---|
| **Personio** (лидер HR-систем МСБ в DACH) | `https://<аккаунт>.jobs.personio.de/xml` (и `.com`), без ключа; все открытые вакансии с описанием, юрлицо (`subcompany`), офис (город), дата | функция «XML feed» работодатель включает сам — для размещения вакансий на других сайтах (developer.personio.de/docs/retrieving-open-job-positions; support.personio.de, FAQ по XML); robots.txt доски — 404 | **выбран первым** |
| Lever | `api.lever.co/v0/postings/<аккаунт>` (EU: `api.eu.lever.co`) | «These jobs may be scraped by third parties» (github.com/lever/postings-api) | разрешено; следующий кандидат |
| Greenhouse | `boards-api.greenhouse.io/v1/boards/<доска>/jobs` | «Job Board data is publicly available», GET без ключа | адаптер уже есть (§13), вход обнаружения — позже |
| SmartRecruiters | Posting API | нужен API-ключ или OAuth | не подходит без договора |
| Recruitee, Teamtailor, JOIN, softgarden | публичные страницы и API карьерных сайтов | официальных условий для сторонних читателей не найдено | не проверено до конца |

**Распространённость** в SK/AT по индексу Common Crawl измерить не удалось: из сред ассистента индекс
недоступен. Её покажет стенд `stand-personio.sh` (число страниц индекса и досок по входу). Открытые
данные служб занятости (AMS, ÚPSVaR) не исследовались: после решения владельца о метке «скрытая»
эталон сравнения не нужен.

### 91.3 Адаптер Personio и вход обнаружения

- `adapters/personio/PersonioAdapter` (`@Component`, код провайдера `personio`): лента — один
  XML-документ со всеми вакансиями (`/xml`), постраничности нет. `listPostings` отдаёт вакансии и
  `locationCounts` — офисы всей ленты (для гейта рынка; `reportsCountries() = true`); с областью
  рынка (`MarketScope`) остаются вакансии, у которых хотя бы один офис на рынке — фильтр на нашей
  стороне (у Personio нет фасетов, как у Workday). `getPosting` перечитывает ленту и берёт вакансию
  (отдельной детали у Personio нет): офис → `SourceLocation.raw`, дополнительные офисы →
  `additional`, описание — разделы «заголовок: текст» без разметки (Jsoup), дата — `createdAt`.
  Зарплаты и страны в ленте нет. `boardProfile`: владелец — имя аккаунта, описание — юрлица ленты
  (`subcompany`) → обычная проверка принадлежности (§79): юрлицо не содержит имени аккаунта или
  есть признак агентства — ручная проверка.
- `adapters/personio/PersonioBoard`: доска по адресу любой её страницы — поддомен
  `<аккаунт>.jobs.personio.de|com`; ключ дедупа — аккаунт (один аккаунт на двух доменах — одна доска).
- `xml/SafeXml.parseDom` — разбор DOM тем же защищённым парсером (без DOCTYPE и внешних сущностей),
  ошибки — исключением без печати в stderr. Нужен, чтобы разбор XML оставался только в пакете `xml`
  (правило ArchUnit §86).
- **Вход обнаружения обобщён.** `discovery/cc/CcInput` (перечисление): код входа (ключ курсора),
  шаблон адресов индекса, провайдер, разбор адреса в доску — `WORKDAY` (`cc-workday`, прежний курсор),
  `PERSONIO_DE`, `PERSONIO_COM`. `CcHarvestScheduler` обходит включённые входы
  (`app.discovery.cc.inputs`, вместо `url-pattern`), у каждого свой курсор, аренда и бюджет страниц;
  `HarvestedBoard` — общий вид доски для накопителя (`HarvestStore.recordPage`).
- **Гейт рынка:** офисы Personio — города; список `location-terms` дополнен городами и землями
  Австрии и Словакии (Klagenfurt, Villach, Wels, St. Pölten, Bregenz, Trnava, Nitra, Prešov…).
  Имя провайдера `personio` заводится при первом подключении (`EmployerSourceRegistrar`), миграция не нужна.

**Ограничения.** Страна у вакансий Personio не заполняется (в ленте только город) — фильтр ленты по
стране их не видит; нужен справочник «город → страна» (следующий шаг). Деталь каждой вакансии
перечитывает всю ленту (ленты небольшие, запросы — в пределах дневного бюджета и темпа).

**Тесты.** `PersonioAdapterTest` — разбор ленты (офисы без повторов, описание без разметки), офисы
всей ленты и отбор по рынку, деталь из ленты и «вакансии нет», юрлица для проверки принадлежности,
отказ на DOCTYPE, доска по адресу. `CcHarvestSchedulerTest` — входы Personio, дедуп между доменами,
свой курсор у каждого входа; прежние тесты — на `CcInput.WORKDAY`. `HarvestStoreIntegrationTest` —
на `HarvestedBoard`. Проверено ассистентом: разбор ленты и отказ на DOCTYPE — на JDK (копия кода),
Checkstyle — 0 нарушений; компиляция всего проекта не проверялась (Maven Central недоступен).

**Стенд.** `target/stand-personio.sh` (вне git): только входы Personio, MIN минут (15), отчёт
`target/stand-personio.txt` — размер индекса, доски, итог гейта, подключённые, собранные вакансии.

**Документы.** Реестр оснований (Personio, Lever, Greenhouse), план (A4, A5), `current-state.md`,
README — строка статуса: вход и адаптер Personio, проверка karriere.at выключена (§0.2).

## §92 — Проверка досок: входы по очереди

**Проблема (стенд §91).** Передача досок в проверку (`BoardFanOut` → `HarvestStore.lockNewBoards`)
брала самые старые доски `NEW` без учёта входа. В накопителе лежали сотни досок Workday из прошлых
прогонов, и за 15 минут все 100 заданий ушли на них: ни одна из 52 найденных досок Personio до гейта
не дошла. В обычной работе так же: каждый новый вход (Personio, дальше Greenhouse, Lever) ждал бы,
пока не разберётся весь хвост старого.

**Исправление.** `lockNewBoards` берёт доски из входов по очереди: первая доска каждого входа, затем
вторая и так далее (`row_number() OVER (PARTITION BY input_code ORDER BY id)`), внутри входа — по
порядку обнаружения. Оконная функция — во вложенном запросе: PostgreSQL не разрешает `FOR UPDATE`
вместе с ней. Бюджет прохода прежний (`max-fan-out`, 20 досок за 10 минут), теперь он делится между
входами. Запрос проверен ассистентом на PostgreSQL 16 (схема теста: из 3 досок Workday и 1 Personio
при лимите 2 взяты по одной).

**Тест.** `HarvestStoreIntegrationTest.newBoardsAreTakenFromEachInputInTurn`.

**Не вошло.** Приоритет входов (сейчас поровну); увеличение бюджета проверки. README — без изменений.

## §93 — Цель: от региона и профиля к карьерной странице (только документация)

**Решение владельца.** Приложение ищет вакансии на карьерных страницах компаний заданных регионов и
профиля; площадки — во вторую очередь; ручной ввод компаний — дополнительная опция. Собственные
карьерные страницы (без вендорной системы найма) входят в охват через стандартную разметку
schema.org `JobPosting`. Сначала — документы и план, код — следующими срезами.

**Почему пересмотр.** Код шёл «от вендора»: все доски Workday/Personio из индекса Common Crawl с
отсевом по рынку. Результат — в основном чужой рынок (стенд §91: 27 из 42 досок Personio вне рынка,
1 подключение — кадровое агентство), а компании Словакии и Австрии попадают, только если случайно
сидят на этих вендорах.

**Изменено.**
- Бизнес-ТЗ §7.1 — обнаружение «от региона и профиля к карьерной странице»; граница охвата —
  вендорные системы найма и собственные страницы с `JobPosting`.
- Технический документ — §5 (входы обнаружения: основной — от региона и профиля, вендорный гарвест
  — дополнительный), ADR-19.
- План — приоритет R (R1–R7), прежний приоритет A — дополнительный вход.
- `current-state.md` — реестр оснований: RPO, Firmenbuch HVD, Wikidata, страницы с `JobPosting`.

**Исследование источников списка компаний (2026-09-25).**
- **Словакия — RPO:** открытый REST API без ключа (rpo.minv.sk/rpo-api-doc.html), CC BY 4.0;
  есть основной вид деятельности (`statisticalCodes.mainActivity`, SK NACE) и поиск по нему; сайта и
  контактов нет.
- **Австрия — Firmenbuch HVD** (EU 2023/138): API JustizOnline по бесплатному ключу, CC BY 4.0,
  ежедневное обновление; наличие кода деятельности не подтверждено.
- **Сайт компании** в реестрах не указан: берётся из Wikidata (CC0, для заметных компаний) или
  подбирается и подтверждается реквизитами на сайте — в Словакии и Австрии сайт компании обязан
  показывать IČO / номер Firmenbuch (выходные данные), что даёт проверку принадлежности.

**Метка «скрытая»** — вопрос владельца открыт (поле `coverage`, фильтр, код покрытия); в документах
пока не менялась.

Код не менялся. README — цель и текущий вход (§0.2: изменилось заявленное назначение).

## §94 — Регламент: каждый срез двигает цель (только документация)

По замечанию владельца: направление правилось несколько раз, а работа сваливалась в детали —
ассистент шёл от последнего результата стенда, а не от цели. В регламент сессии §2 добавлено правило:
срез начинается строкой «какой пункт цели двигает»; дефекты вне основного пути — в план, не в
«следующие шаги»; при смене направления сначала пересматриваются план и документы. §3 — «следующие
шаги» только из основного приоритета. Код не менялся.

## §95 — Путь «от региона»: IT-компании Словакии из реестра RPO и замер на выборке

**Двигает цель:** план R1 (список компаний Словакии) и R7 (замер пути на выборке); R3–R5 — в
виде первой версии, нужной для замера.

**Что сделано (job-worker, пакет `region`; job-api — V32).**
- `V32__registry_company.sql` — `registry_company`: юрлицо реестра (страна, реестр, IČO, название,
  город, основной вид деятельности) и итог проверки пути: сайт, карьерная страница, её тип
  (`career_system`), число вакансий и вакансий ниши, пояснение, время проверки.
- `RpoClient` — открытый API реестра RPO (`https://api.statistics.sk/rpo/v1`, CC BY 4.0): поиск
  действующих юрлиц по слову основного вида деятельности и городу (API требует фильтр и отдаёт до
  500 записей), карточка — код вида деятельности (SK NACE). Из истории записи берётся действующее
  название (без `validTo`).
- `RegionRun` (leader-lock 1008, тик `RegionTrigger`, `app.region`, по умолчанию выключен): (1)
  импорт, пока в выборке меньше `import-limit` (50) — поиск по словам × городам, карточка на каждую
  новую компанию (не больше `lookups-per-pass` за проход), в выборку — только код, начинающийся
  на `62` (программирование, IT-услуги); (2) проверка `check-batch` компаний.
- `SiteLocator` (R3) — в реестре сайта нет: кандидаты домена из названия без правовой формы и
  диакритики (`softecon.sk`, `pixel-federation.sk`, первое слово, `.com`); сайт принят, только если
  на главной или `/kontakt`, `/contact` стоит IČO — сайт словацкой компании обязан его показывать.
- `CareerPageInspector` (R4, R5) — ссылка на раздел вакансий на главной (kariéra, career, jobs,
  práca, voľné miesta…); тип: система найма по адресу ссылки или по ссылкам/фреймам страницы (Workday,
  Personio, Greenhouse, Lever, Teamtailor, Recruitee, SmartRecruiters, Recruitis, Traffit, Teamio…);
  разметка schema.org `JobPosting` в JSON-LD (в том числе `@graph`) — `schema-org`; иначе `own-page`;
  нет ссылки — `none`. Вакансии считаются для `schema-org` (по разметке) и Personio (адаптером);
  ниша — по заголовкам (`NicheFilterProperties`). Вакансии не сохраняются — это замер.
- `CompanyCheck` — одна компания: сайт → страница → пояснение; ошибка не останавливает проход.

**Ограничения первой версии.** Слово поиска RPO по виду деятельности не проверено на живом API
(поиск по коду `6210` дал 0; ассистенту не хватило запросов, чтобы проверить слова) — если стенд
импортирует 0, меняются `activity-terms`. Домен угадывается по названию — компании с другим доменом
не найдутся (доля покажет стенд). robots.txt сайтов не проверяется — для замера на десятках сайтов,
до промышленного сбора — обязательно.

**Тесты.** `RpoClientTest` — действующие название, IČO, город из истории; `SiteLocatorTest` — кандидаты
домена, IČO с пробелами, сайт только с IČO; `CareerPageInspectorTest` — ссылка по тексту и адресу,
система по хосту, страница с `JobPosting` (2 вакансии, 1 в нише), встроенная система, своя страница,
нет раздела; `RegionRunTest` — в выборку только код 62, полная выборка не импортирует. Проверено
ассистентом: кандидаты домена на JDK (ESET, Sygic, Pixel Federation, GlobalLogic), Checkstyle — 0;
компиляция проекта не проверялась (Maven Central недоступен).

**Стенд.** `target/stand-region.sh` (вне git, нужна V32): воркер с проходом раз в минуту, MIN минут
(20), отчёт `target/stand-region.txt` — сколько импортировано, у скольких найден сайт, типы карьерных
страниц, вакансии и ниша, список компаний.

**Документы.** План (R1, R3–R5, R7), `current-state.md`, реестр оснований (RPO — читаем), README.

## §96 — Путь «от региона»: отказ карточки реестра не срывает импорт

**Двигает цель:** R1 (список компаний Словакии) — стенд §95 не импортировал ни одной компании.

**Стенд §95.** Поиск RPO по слову «programovanie» отвечает (дальше шли запросы карточек), но
карточки `/entity/{id}` 26 раз не уложились в тайм-аут чтения (15 с). Отказ одной карточки прерывал
весь проход: импорт и проверка стояли, все 12 проходов за 20 минут кончились ошибкой.

**Исправление.** `RegionRun`: отказ карточки — пропуск компании с предупреждением в логе; после
`MAX_FAILED_LOOKUPS` (3) отказов подряд импорт прохода прерывается, проход переходит к проверке
сайтов, импорт продолжится на следующем тике. Тест `RegionRunTest.unreadableCardIsSkippedAndImportGoesOn`.

**Не вошло.** Если карточки RPO медленные всегда — следующим шагом отказ от карточки: доверять
поиску по виду деятельности или брать код из выгрузки открытых данных. Решение — по стенду.

## §97 — План развития переработан целиком (только документация)

**Решение владельца.** Последовательность развития — заново, от начала до конца; реализованное
проверяется по новому плану. Принципы: всё автоматически; вакансия существует, только если она на
карьерной странице компании; нет страницы — компания пропускается; размер и профиль компании — не
критерии; поиск сайта — искать метод, находить все компании сразу не нужно; страница «Kariéra» без
разметки пропускается; интерфейс, сложные фильтры и прочий «глянец» — только после того, как ядро
чётко работает.

**Изменено.** `roleorienta-plan-korrektirovka.md` переписан: принципы (§0), ядро K1–K8 с
измерителями (§1), расширение E1–E3 (§2), критерий «ядро работает» (§3), глянец после ядра (§4),
таблица «что реализовано» по пунктам (§5), расхождения с бизнес-ТЗ и техническим документом —
править после одобрения (§6). `current-state.md` — точка остановки; регламент §2 — основной
приоритет «ядро K».

**Итог проверки (план §5).** Ядро реализовано частично: K1–K6 — части есть (реестр RPO с лишним
фильтром по виду деятельности, поиск сайта угадыванием, тип страницы, адаптеры, сбор, надёжность
доставки), K7 (единый отчёт) — нет, K8 — есть. Большая часть «глянца» (вход, маркеры, подписки,
уведомления, отклики, собеседования, жалобы, фильтры ленты) построена заранее — заморожена. Покрытие
площадками — выключено, к удалению.

Код не менялся. README — без изменений до одобрения плана.

## §98 — Ручной ввод — третий источник; «скрытая» исключена; документы приведены к плану (только документация)

**Решение владельца.** Ручной ввод компании — только третий вариант после автоматического поиска
по реестрам и по площадкам, опция по желанию клиента. Маркер «скрытая» смысла не имеет — исключён.
Аудит — после всех исправлений.

**Изменено.**
- План развития: принцип 1 — порядок источников компаний (реестры и индексы систем найма →
  площадки с разрешением → ручной ввод по желанию клиента, тем же автоматическим путём); принцип 8
  — признака «скрытая» нет; E4 — ручной ввод компании; из «после ядра» (§4) ручной ввод убран;
  таблица §5 — строки «Вне продукта» (код покрытия) и «Вне ядра» (очередь подтверждения); §6 —
  расхождения документов устранены.
- Бизнес-ТЗ: ревизия 2026-09-25 — убраны признак покрытия, фильтр «только скрытые», «скрытые» в
  дайджесте, роль куратора с подтверждением источников, непустой первый экран; §7.1 — обнаружение
  полностью автоматическое, без фильтра по виду деятельности и размеру, вакансия — только с
  карьерной страницы, порядок источников; §8 — нецели (покрытие, ручное подтверждение, страницы без
  разметки); §11 — «сначала ядро, потом остальное» вместо Этапов 0–2.
- Технический документ: глоссарий, модель (`CoverageAssessment` — исключено), инварианты, §5
  (обнаружение полностью автоматическое, порядок источников, перепроверка страниц, robots.txt),
  `DISCOVER_EMPLOYER`, дедупликация без ручной проверки, API (что построено и к удалению/замене),
  приёмка, §14 (порядок — план развития), пересмотр ADR-6/14/17/18/19, ADR-15 отменён, **ADR-20**
  (сначала ядро; всё автоматически; без «скрытой»; ручной ввод — третий источник).
- `current-state.md`, README — цель и порядок источников; счётчик срезов — 11.

**Площадки.** По словам владельца («автоматический поиск по реестрам и по платформам»; «наличие
позиции — по наличию на кадровой странице») площадки — второй источник **названий компаний**, а не
вакансий: вакансии берутся только с карьерной страницы компании (принцип 2). Так записано в плане
(E3), бизнес-ТЗ (§7.1, §8, §9), техническом документе (§4 — глоссарий, `Source`, `CompanySource`, контракт адаптера; §5, §6, §9, ADR-16, ADR-20) и README.

**Аудит исправлений.** Независимая проверка всех изменённых документов (поиск остатков «скрытая»,
покрытия, ручного подтверждения, профиля компании, пилота; сверка ссылок K/E/§/ADR между
документами; таблицы). Найдено и исправлено: технический документ §9 — ввод компаний «не меняет
данные без административного подтверждения» → тот же автоматический путь; §10 — метрика «отправлено
на подтверждение»; README — «площадка сравнения для Словакии» и «Этап 1 (пилот)»; площадки как
источник вакансий (план E3, бизнес-ТЗ, технический документ §4/§6); гипотеза «вакансии только на
сайтах компаний» (требовала сравнения с площадками) → доля компаний со структурными страницами по
отчёту K7; ссылки на отменённый пилот в бизнес-ТЗ; в API добавлен построенный
`POST /api/v1/admin/employer-candidates/discover`; «кураторская роль» → административная; вход
Common Crawl в Этапе 1 — дополнительный; прежнее противоречие §11 «worker без брокера на Этапе 1»
(с ADR-10); ссылки на закрытый Этап 0; `current-state.md` — очередь кандидатов, `stand-coverage.sh`,
karriere.at помечены к удалению.

**Не менялось.** Код. Построенный код покрытия (V29, пакет `coverage`, поле и фильтр `coverage` в
ленте, строка в дайджесте) и очередь подтверждения кандидатов остаются до отдельных срезов —
изменение API требует разрешения владельца.
