...
Дата: 2026-09-22
Статус: гейт рынка в обнаружении (§56, A2-минимум) — WorkdayAdapter читает фасет стран (Accept-Language: en-US, вложенные группы), PostingsPage.countryCounts; DISCOVER_EMPLOYER: есть публикации в Slovakia/Austria → HIGH, нет → OUT_OF_MARKET (новое состояние кандидата, без очереди подтверждения); неизвестно → прежнее правило. Без доп. запросов. Дальше: вежливость к Workday (B1-минимум), затем сбор с фильтром по стране. Компиляция/тесты в среде ИИ не запускались — их выполняет владелец.
Прежний статус: автоматический вход Workday по Common Crawl — V26, HarvestStore, CcHarvestScheduler, выключен до B1/B2 (§55); вход Workday — Common Crawl вместо CT, CommonCrawlIndexClient и WorkdayBoard.fromCareerUrl (§54); Workday site-резолв по словарю через cxs (§53, A1b-1); CT-клиент CertSpotterClient (§52, A1a); Workday сквозь обнаружение end-to-end на заглушке (§51); первый энтерпрайз-адаптер Workday (§50); протокол Этапа 0 (§49); ревизия документации — вендор-инклюзивный охват и ADR-17/18 (§48); интеграция персональных маркеров в ленту (§31); персональные маркеры (§30); аутентификация (§29); REST-чтение и фильтры (§26–28); извлечение по таксономии/опыт/stance (§20–25); нормализация зарплаты/локации и история (§17–19); конвейер сбора (§13–16)
---
