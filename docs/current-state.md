...
**Обновлено:** 2026-09-24, после §83.
**Срезов с последнего аудита: 7** (последний аудит — §76).
...
6. **Покрытие (A5, §82).** `CoverageCheck` (worker, leader-lock 1005, тик `CoverageTrigger`,
   ...
   Компании со старым именем-slug переименовывает `CompanyNameBackfill` (lock 1006, §83).
...
- A5 — Австрия (karriere.at, §81–§82). Далее: площадка для Словакии;
