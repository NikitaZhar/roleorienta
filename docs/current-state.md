...
**Обновлено:** 2026-09-25, после §92.
**Срезов с последнего аудита: 5** (последний аудит — §87).
...
бренда (~3 мин, `stand-brand.txt`); `stand-personio.sh` — вход Personio: доски из индекса, гейт, сбор
...
| karriere.at (Австрия) | страница выдачи по работодателю, 1 запрос / 10 с | **Nutzungsbedingungen п. 2.8.3: «Sämtliche Formen der automatisierten Auswertung unserer Plattform sind verboten»**, действуют для всех посетителей (п. 1.3) (2026-09-25). Прежняя проверка (§82) смотрела только AGB | факт наличия: id, состояние, дата | **запрещено** без письменного разрешения |
...
## Статус плана ([план работ](roleorienta-plan-korrektirovka.md))

- Сделано: A1, A7, B1, B2, C4, D4; частично: A2 и A3 (внутри Workday), A4 (Workday), A5 (Австрия).
- Проверка покрытия по karriere.at **выключена** (§91, условия площадки). Владелец: метка «скрытая»
  мало что добавляет — судьба покрытия (A5: код, поле `coverage` в API, фильтр) ждёт решения.
- Personio — вход обнаружения и адаптер (§91). Следующий срез — по итогам стенда `stand-personio.sh`.
- A6 — снимки в S3 (MinIO + AWS SDK v2); удаление `CompanyNameBackfill`, когда slug-имён не останется (на 2026-09-25 — 47).
- Открыто: A3 между провайдерами, A4 (SuccessFactors, SmartRecruiters), B3–B5 (безопасность входа,
  потолок попыток outbox), C1–C3, C5–C7, D1–D3, D5–D6.
- Этап 0 закрыт (протокол — `stage-0-protocol.md`, §89).
