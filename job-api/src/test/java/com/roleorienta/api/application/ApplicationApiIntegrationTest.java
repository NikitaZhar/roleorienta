...
/**
 * Интеграционный тест откликов и заметок (§41) на реальном PostgreSQL (Testcontainers,
 * схема — Flyway) через MockMvc с цепочкой безопасности. Проверяет создание/список/
 * карточку с заметкой и приватность на <b>втором</b> пользователе (A23).
 */
@SpringBootTest
...
class ApplicationApiIntegrationTest {
    ...
    private long createApplication(MockHttpSession session) throws Exception {
        MvcResult result = mockMvc.perform(post("/api/v1/applications").with(csrf()).session(session)
